...
Portfolio_analysis <- function(res_list_portfolio,from,to,mapping_method,mapped_status,FX_split=TRUE){
  ...
  # [DEBUG] ACE 종합채권 03-13~16 pre-cumprod
  cat("\n[TRACE_norm_sec] 기초정보_weight_performance ACE 종합채권 03-13~16 (pre-cumprod):\n")
  print(기초정보_weight_performance %>% filter(sec_id=="KR7356540005", 기준일자 >= as.Date("2026-03-13"), 기준일자 <= as.Date("2026-03-16")) %>% select(기준일자, sec_id, `수익률(FX_제외)`, `수익률(FX_포함)`))
  ...
  기초정보_weight_performance %>%
    ...
    { cat("\n[TRACE_norm_sec] after reframe(cumprod) ACE 종합채권 03-13~16:\n"); print(filter(., sec_id=="KR7356540005", 기준일자 >= as.Date("2026-03-13"), 기준일자 <= as.Date("2026-03-16")) %>% select(기준일자, sec_id, `수익률(FX_제외)`, `수익률(FX_포함)`)); . } %>%
  ...
  cat("\n[TRACE_norm_sec] 최종 norm_performance_by_sec ACE 종합채권 03-13~16:\n")
  print(norm_performance_by_sec %>% filter(sec_id=="KR7356540005", 기준일자 >= as.Date("2026-03-13"), 기준일자 <= as.Date("2026-03-16")) %>% select(기준일자, sec_id, `수익률(FX_제외)`, `수익률(FX_포함)`))
