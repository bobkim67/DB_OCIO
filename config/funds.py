...
# 투자 위험 등급 (1~6, 투자설명서 공시값 — DB 미보유라 하드코딩).
# 2026-07-14 사용자 확정: 08K88=2, 나머지 전부 4. 운용보고 PPT 표지 게이지에 사용.
FUND_RISK_GRADE = {'08K88': 2}
DEFAULT_RISK_GRADE = 4
RISK_GRADE_NAME = {1: '매우높은위험', 2: '높은위험', 3: '다소높은위험',
                   4: '보통위험', 5: '낮은위험', 6: '매우낮은위험'}
...
FUND_BM = {
    ...
    # BM 미설정 펀드: 08N33, 08N81, 08P22 (SAA 비교), 2JM23 (벤치마크 없음 — 아래 참조)
    # → Tab 0에서 BM 없이 표시
}
...
# 벤치마크 미표시 펀드 (AP 단독)
# ============================================================
# 2026-07-29 사용자 확정: **2JM23 은 BM/SAA 를 쓰지 않고 AP 만 표시**한다.
#   배경 — solution.saa_bm_components 에 2JM23 SAA 가 2버전(2016-03-24 / 2025-12-30) 있으나
#   버전 선택이 '기간말 기준 단일 셋'이라 설정후 분석이 2025 구성을 2016 년까지 소급했다
#   (SAA +356.83% vs AP +148.76%, 초과 -208%p). 구성 자체가 사후 부여된 참조선이라
#   장기 비교가 성립하지 않아 벤치마크를 두지 않기로 했다.
#   ※ DB 행(saa_bm_components)·FUND_MP_DIRECT 항목은 이력 보존용으로 남긴다 —
#     아래 집합이 SAA / proxy / MP목표비중 **전 경로**를 차단한다.
FUND_NO_BENCHMARK = {'2JM23'}
