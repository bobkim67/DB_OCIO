...
calculate_BM_results_bulk_for_users_input<- function(dataset_id_vector, dataseries_id_vector, region_vector,
  ...
  Source_factset %>% 
    ...
    full_join(F_USDKRW_Index %>%
                mutate(`return_F_USD/KRW`=`F_USD/KRW`/lag(`F_USD/KRW`)-1), by = join_by(기준일자)) %>%
    arrange(기준일자) %>% 
    bind_rows(tibble(기준일자 = today()-days(1))) %>% 
    group_by(기준일자) %>%
    filter(row_number()==1) %>%
    ungroup() ->Source_combined
  
  # ... (이하 계산 로직은 수정 없이 그대로 사용) ...
  
  results <-
    Source_combined %>%
    select(기준일자, for_pulling_universe_data$symbol,`USD/KRW`,`return_USD/KRW`,`return_F_USD/KRW`) %>%
    ...
    mutate(`return_F_USD/KRW_list` = map(`return_F_USD/KRW`, ~ rep(.x, nrow(for_pulling_universe_data)))) %>%
    mutate(daily_return_list = pmap(list(raw_data_list,lag_raw_data_list,`return_USD/KRW_list`,`return_F_USD/KRW_list`),
                                    ~ {
                                      # 환노출비중 = (1-헤지비중)* (KR은 무조건 USD/KRW곱하는것  제외)
                                      ...
                                      #환헷지비용 달러선물지수로 정확히 계산하기
                                      if(hedge_cost_strictly==TRUE){
                                        realistic_hedge_cost <- (for_pulling_universe_data$hedge_ratio)*(..3-..4)
                                        return_vector_hedge_ratio_considered <- (1+return_vector_hedge_ratio_considered)*(1+realistic_hedge_cost)-1
                                      }
    ...
    select(-c(`return_USD/KRW_list`,`return_F_USD/KRW_list`,`return_F_USD/KRW`)) %>%
...
#        dataseries_id="15") %>%
#  slice(2)
backtesting_for_users_input<- function(backtest_prep_table,rebalancing_option = "A",`trading_cost(bp)` = 0,hedge_cost_strictly = FALSE){
  ...
  backtest_prep_table %>% 
    ...
    mutate(리밸런싱마감일= if_else(is.na(리밸런싱마감일),today()-days(1), 리밸런싱마감일 )) %>%
    ungroup() %>%
...
backtesting_for_users<- function(backtest_prep_table,rebalancing_option = "A",`trading_cost(bp)` = 0, hedge_cost_strictly = FALSE){
  print("### backtesting_for_users 함수 시작 ###")
  print("입력 데이터:")
  print(head(backtest_prep_table)) 
...
return_res_tables<- function(result_core_prep,
  ...
  # result_core_prep<- res[[2]] %>% filter(기준일자>= ymd(from)) %>% 
  ...
  results_주간수익률 %>% 
    ...
    mutate(
      ...
      `18M` = return_ref_date_v2(분석종료일,18,"M"),
