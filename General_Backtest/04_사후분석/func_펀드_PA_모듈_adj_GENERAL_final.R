...
ITEM_NAME_8004 <- 
  tbl(con_dt,"DWPM10530") %>%
  select(STD_DT,ITEM_NM,ITEM_CD) %>%
  #inner_join(tbl(con_dt,"DWPM10510"), by = c("FUND_CD", "IMC_CD", "STD_DT")) %>%
  filter(STD_DT>="20210101") %>% 
  #filter(FUND_CD %in% c('07G04','07G02','07G03')) %>%
  select(ITEM_CD, ITEM_NM) %>% 
  filter(!(str_detect(ITEM_NM,"미지급")|str_detect(ITEM_NM,"미수"))) %>% distinct() %>% 
  collect() %>% 
  bind_rows( tibble(ITEM_CD ="000000000000",ITEM_NM="기타비용")) %>% 
  group_by(ITEM_CD) %>%  
  filter(row_number()==max(row_number())) %>% #가장 최근 종목명 가져오기.(종목명 바뀌는 경우 존재)
  ungroup()
...
모펀드_mapping %>%
  ...
  left_join(
    ...
    historical_universe_DWPM10530_TOTAL %>% select(-FUND_CD) %>% distinct() %>% 
      ...
      rename(운용모펀드= ITEM_CD),by = join_by(FUND_CD  == 모펀드)
  ) %>%
