...
| v1.7 | 2026-04-30 | **P1-④ Step 1 진단 PASS, Option B 채택 (legacy backfill 미진행).** legacy final 3건 (2026-04/_market not_backfillable, 2026-Q1/08K88·4JM12 manual_review_required) 전부 실제 `debate_run_id` 부재. synthetic ID 백필 미채택 (감사추적 훼손). legacy final 은 `approved_debate_run_id=null` 유지 + timestamp fallback 정책 그대로 유지. 운영 파일 변경 없음. 진단 산출물 `debug/p1_4_diagnose.py` + `debug/p1_4_diagnosis.json` (debug 전용). 정책 §10 신설. |
| v1.8 | 2026-04-30 | **P3 트랙 종료 + 2026-04 운용보고 사이클 완료.** P3-1 LinkedMarketEnrichmentDTO (펀드 viewer 의 시장 evidence fan-out, 시장 lineage 분리 검증). P3-2 wiki/graph prompt 보강 (graph_paths TARGET_MIN=6/MAX=10/conf≥0.3, wiki_retriever 01_Events·02_Entities·05_Regime_Canonical 키워드 retrieval). P3-3 + P3-3.1 hotfix asset coverage guardrail (8 자산군 covered/weak/missing 분기, raw count prompt 미노출 + bucket only, dominant_topic_group cluster, false-positive include/exclude). P3-4 + P3-5 + P3-4.1 03_Assets/04_Funds enrichment builder (8 asset + 7 fund standard pages, internal link 131건, boundary asset 700ch + disclaimer + chips, filler 회피). 2026-04 _market modern final (run_id=`8cb20bf8…`) + 7펀드 modern final 정상 approve, matched_by_id + ev=12/rn=3/ic=3 + 누출 0. dry-run 운영 wiki 0 변경 (commit_enrichment_plan 호출은 별도 GO 후). pytest 202/202 PASS. |
...
| ④ **legacy final backfill 가능성 검토** | **2026-04-30 Step 1 진단 완료 — PASS, Option B 채택 (백필 미진행).** 진단 대상 final 3건 (2026-04/_market, 2026-Q1/08K88, 2026-Q1/4JM12) 전부 legacy. 모든 source (draft/jsonl/debate_logs) 의 `debate_run_id` 가 None — P1-① 도입 이전 산출물. 자동 backfill candidate 0건. **결정**: synthetic ID 부여 시 실제 lineage 가 복구된 것처럼 보여 감사추적 의미 훼손 → 백필 미진행, legacy timestamp fallback 정책 그대로 유지. Step 2 dry-run / 실 migration 도 진행하지 않음. 진단 산출물 `debug/p1_4_diagnose.py` + `debug/p1_4_diagnosis.json` (debug 전용, 운영 산출물 아님). 상세 정책은 §10 참조. |
...
## 10. Legacy Final Backfill 정책 (v1.7 — P1-④ Step 1 PASS, Option B 채택)

P1-① (v1.4) 도입으로 신규 final 은 `approved_debate_run_id` 기반 strict matching 을
사용한다. **이 섹션은 P1-① 이전에 승인된 legacy final 의 처리 정책을 명시한다.**

### 정책 (must)

- **Legacy final 은 `approved_debate_run_id` 가 `null` 일 수 있다.** 이는 정상 상태이며
  결함이 아니다.
- **P1-① 도입 이전 산출물에는 실제 `debate_run_id` 가 존재하지 않으므로, 임의 synthetic
  ID 를 `approved_debate_run_id` 에 백필하지 않는다.**
- **Legacy final 은 timestamp fallback guard 로만 안전성을 판단한다** (§4 Lineage 정합성
  가드 2번 항목). draft/jsonl timestamp 가 `approved_at` 이하일 때만 enrichment 결합
  허용.
- **신규 final 은 `approved_debate_run_id` 기반 strict matching 만 사용한다** (§4 1번
  항목). timestamp fallback 금지.
- **Synthetic ID 는 실제 debate run lineage 가 아니므로 `approved_debate_run_id` 로
  사용하지 않는다.** lineage 무결성 / 감사추적 보존 목적.

### Step 1 진단 결과 (2026-04-30, read-only)

진단 스크립트: `debug/p1_4_diagnose.py` (debug 전용, 운영 코드 아님).
산출물: `debug/p1_4_diagnosis.json` (debug 전용 진단 보고서, 운영 산출물과 무관).

| period | fund | category | 사유 |
|--------|------|----------|------|
| 2026-04 | _market | **not_backfillable** | 현재 `_market.draft.json.generated_at=2026-04-17T11:55:27` 가 `final.approved_at=2026-04-13T16:11:18` 보다 4일 이후 → 원 승인 draft 가 이후 새 debate 로 덮어써져 소실. 복구 불가능. |
| 2026-Q1 | 08K88 | **manual_review_required** | `draft.generated_at == final.generated_at` (정확 일치) 으로 동일 산출물로 보이지만, draft / jsonl / debate_logs 어디에도 실제 `debate_run_id` 가 없음. 백필 미진행. |
| 2026-Q1 | 4JM12 | **manual_review_required** | 동일 사유 (draft/final gen_at 정확 일치 + run_id 부재). 백필 미진행. |

전체 final 3건 / legacy 3건 / auto_backfill_candidate 0건 / already_modern 0건.

### 검토했으나 채택하지 않은 옵션

- **Option A (synthetic ID 부여)**: legacy final 3건에 deterministic synthetic ID
  (예: `legacy-{sha1(period+fund+approved_at)[:8]}`) 부여. **미채택** — 실제 lineage 가
  복구된 것처럼 보여 감사추적 의미 훼손.
- **Option C (forced re-debate)**: 과거 시점 시장상황을 재구성해 debate 재실행 후
  approve. **미채택** — 사실상 불가능 / 별도 retro 작업 필요.

### 운영 처리 (현재 상태)

- 운영 final/draft/jsonl/debate_logs 파일 변경 없음.
- `approved_debate_run_id=null` 인 legacy final 은 그대로 유지.
- API 의 legacy timestamp fallback 정책 유지 (§4 Lineage 정합성 가드 2번).
- Step 2 dry-run migration / 실제 migration 진행하지 않음.
