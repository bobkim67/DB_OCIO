...
def check_block_rules(fund_code: str, block: str, text: str) -> list[str]:
    ...
    # 구조 — 규칙이 2문장 110~170자를 요구한다
    n_sent = text.count('다.')
    if n_sent != 2:
        out.append(f'문장 수 {n_sent} (규칙 2문장)')
    if not 110 <= len(text) <= 170:
        out.append(f'분량 {len(text)}자 (규칙 110~170자)')
    if not any(k in text for k in _PLAN_REASONING):
