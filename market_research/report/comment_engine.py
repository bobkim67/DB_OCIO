...
SEEDED_BLOCKS = {
    ...
    # E = DB생명(4JM12) — 2026-08-06 사용자 스펙. 6슬롯이라 D 와 구조가 다르다.
    ...
    'E': [
        ...
        ('펀드성과', '**BM 대비 수익률**로 시작하세요 — 펀드 수익률, BM 수익률, 초과폭(bp). '
                     '이어서 **어떤 운용 결정이** 상대성과를 끌어올렸고 무엇이 발목을 '
                     '잡았는지 씁니다. 2~3문장, 170~250자.'),
        ('운용계획', '향후 자산군 스탠스와 스타일 틸팅 방향. 2~3문장, 120~200자.'),
    ],
    'K': [
...
#
# ★ 1차 규칙은 **금지 목록뿐**이라 실패했다. 모델이 덜어내는 방향으로만 최적화해
#   "국내주식을 순매수하여 비중을 확대하였으며, 국내채권은 일부 순매도하였습니다"
#   처럼 **사실 나열만 남고 판단 근거가 통째로 빠졌다**. 운용계획의 값어치는 거기 있다.
#   → 금지 목록에 **긍정 구조(2문장: 판단→실행 / 재원→축소사유)** 와 **모범 예시**를 더한다.
#     시드 생성에서 `_STYLE_ANCHOR`(승인본)로 문체를 잡은 것과 같은 방식이다.
#
# ⚠ 종목명 금지는 **개별 종목명**(ACE 200 등)에 한정한다. 사용자 확정본이 "금 ETF",
#   "국고채 포지션" 을 쓰므로 자산 유형 표기까지 막으면 오히려 발송본과 멀어진다.
BLOCK_EXTRA_RULES = {
    ('2JM23', '계획'): (
        '   ★ 이 블록만의 구조·표현 규칙 (위 자수 지시보다 **이 규칙이 우선**):\n'
        '     · **2~4문장, 110~250자.**\n'
        '         ① 시장을 어떻게 판단했고 그래서 무엇을 했는지 (판단 근거 → 실행)\n'
        '         ② 재원을 어디서 마련했고 그 자산을 왜 줄였는지 (재원 → 축소 사유)\n'
        '         ③ (선택) 향후 어디에 초점을 둘지 — 발송본은 여기까지 씁니다.\n'
        '       ★ "무엇을 했다"만 나열하면 실패입니다. **판단 근거가 반드시 있어야** 합니다.\n'
        '     · 재원은 위에 제공된 자산군별 순매수에서 **순매도로 찍힌 자산군만** 쓰세요.\n'
        '       매매가 없는데 비중만 줄어든 자산군(가격·환율 효과)을 재원으로 쓰면 사실과 '
        '다릅니다.\n'
        '     · **비중·수익률·금액을 쓰지 마세요** — "비중을 11.1%에서 27.7%로" 금지.\n'
        '       (시점 표기는 괜찮습니다 — 발송본이 "11월 이후에는…" 을 씁니다.)\n'
        '     · **개별 종목명·티커 금지** (ACE 200 / ACE 국고채10년 / ISHARES GOLD '
        'TRUST MICRO 등). 자산군(국내주식·국내채권·대체)이나 자산 유형(금 ETF·국고채) '
        '표기는 괜찮습니다.\n'
        '     · **정도부사 금지** — 대폭·크게·급격히·상당히·큰 폭으로·대거.\n'
        '     · 모범 예시 — **2026-07 실제 발송본**입니다. 문체와 인과 밀도의 기준이며 '
        '내용은 재사용하지 마세요:\n'
        '       "당월에는 KOSPI 급락이 반도체 쏠림 해소에 따른 과매도라고 판단해, '
        '국내주식 비중을 확대하였습니다. 재원은 유가발 인플레이션이 실질금리를 밀어올려 '
        '하방 헷지 기능이 약해진 금 ETF와 국고채 포지션을 축소해 마련하였습니다. '
        '향후에는 주가상승에 따른 이익실현에 초점을 맞출 계획입니다. 미국 중간선거가 '
        '있는 11월 이후에는 위험관리를 통해 BM 초과 성과 달성 확률을 높이는데 주력할 '
        '계획입니다."\n'
        '     · 나쁜 예 — 사실 나열뿐이고 판단 근거가 없습니다:\n'
        '       "당월 펀드는 국내주식을 순매수하여 비중을 확대하는 리밸런싱을 '
        '실행하였으며, 국내채권은 일부 순매도하였습니다."'
    ),
}

# 07G07(포맷 K) 블록 규칙 — 2026-09-01 사용자 확정본에서 역산.
# 사용자가 직접 쓴 2026-08 코멘트가 기준이다. 세 블록 모두 **판단이 담긴 서사**이고,
# 수치는 근거로만 쓰며 요인 용어는 성과 블록에서만 쓴다.
BLOCK_EXTRA_RULES[('07G07', '매매')] = """   ★ 서술 순서 (사용자 확정본 구조 — 이 순서로):
     1) 자산군 비중에 유의미한 변동이 있었는지부터. 없으면 "유의미한 변동은 없었으며,
        경기국면모델에 기반한 목표 포트폴리오를 유지하였습니다" 처럼 **유지했다는
        사실과 그 근거(모델·목표 포트폴리오)** 를 씁니다.
     2) 유지·변경한 **포지션의 성격**을 자산군별로. 주식은 어떤 스타일/섹터를 OW 했는지,
        채권은 만기 구성(바벨/불릿, 초장기·중기)을 밝힙니다.
     3) 매매가 있었으면 재원까지 — 무엇을 줄여 무엇을 늘렸는지.
   ★ 개별 종목명은 쓰지 마세요. "반도체 대형주", "미국 성장주", "초장기 국채",
     "중기 국고" 처럼 **자산 유형·스타일 표기**는 씁니다(확정본 표기).
   ★ 모범 예시(2026-08 확정본):
     "당월 중 자산군별 비중에 유의미한 변동은 없었으며, 경기국면모델에 기반한 목표
      포트폴리오를 유지하였습니다. 반도체 대형주 중심의 국내주식과 미국 성장주 OW
      기조를 이어갔고, 채권은 초장기 국채와 중기 국고를 혼합한 바벨 포지션을
      유지하였습니다." """

BLOCK_EXTRA_RULES[('07G07', '성과')] = """   ★ 서술 순서 (사용자 확정본 구조):
     1) 서브 포트폴리오(인컴추구/수익추구) 수익률 문장은 코드가 뒤에 자동으로 붙입니다
        — **쓰지 마세요.**
     2) BM 대비 **자산배분 성과**: 어느 자산군의 OW/UW 가 유리/불리했는지. 괄호로
        (OW)/(UW) 를 붙입니다.
     3) **종목선택 효과**: 어떤 구성이 불리했는지 — 채권은 만기대, 주식은 스타일로.
     4) 전체 포트폴리오가 BM 을 상회/하회한 폭.
   ★ **자산배분과 종목선택을 섞지 마세요.** 같은 자산군이라도 두 층위의 부호가 반대일
     수 있습니다 — 2026-08 실측: 국내채권은 **비중(OW)은 유리, 종목선택(장기물)은
     불리**. "국내채권이 긍정적이면서 부정적" 처럼 쓰면 실패입니다.
   ★ 환율 — **자산군별 수익률은 FX 포함 기준으로 쓰되, 환율 자체에 대한 코멘트는
     반드시 넣으세요** (2026-09-01 사용자 지시). 2026-08 에 원화가 4.0% 절상되며 환이
     -0.70%p 를 눌렀는데 코멘트에 환이 한 줄도 없었습니다.
     · 자산군 수익률 수치는 그대로 **FX 포함 값**을 인용하세요. 해외 자산군 수익률에
       환이 이미 들어가 있습니다 — 환을 별도 항목으로 떼어 자산군 기여와 **나란히
       더하지 마세요**(이중계상).
     · 대신 **환율이 왜 움직였고 그것이 해외자산 원화환산 수익률을 어떻게 눌렀는지/
       밀어올렸는지**를 문장으로 쓰세요. 원인은 [자산군별 시장 movement] 의 환율(FX)
       drivers·인과경로에서 가져옵니다 — "원화 강세" 라고만 쓰고 끝내지 마세요.
       (2026-08 debate: 달러 약세 / 외인 자금 유입 / 한은 인상 기조)
     · [환(FX) 사실] 의 달러/원 변동폭과 달러지수(DXY)를 비교해 **달러 요인인지 원화
       고유 요인인지**까지 밝히세요. ★ 둘이 어긋나면 **수치를 우선**합니다 — 2026-08
       debate 인과경로는 "달러 약세→원화 강세" 였지만 DXY 는 -0.39% 인데 원화는
       -4.01% 로 10배 움직여, 달러 약세만으로는 설명되지 않습니다.
     · 이 펀드는 **환헤지 0%** 라 해외자산이 환에 그대로 노출됩니다 — 현지통화로 오른
       자산도 원화 기준으로는 마이너스가 될 수 있습니다. 해외주식이 "강세"인데 펀드
       수익률은 마이너스인 달이 실제로 그랬습니다(2026-08 해외주식 원화 -1.10%).
     · BM 대비를 말할 때는 **환노출 차이(환 자산배분 효과)** 로 쓰세요 — 절대 기여와
       부호가 반대일 수 있습니다 (2026-08 실측: 절대 -0.70%p 이지만 BM 대비 +0.63%p 유리).
   ★ "자산배분효과 +0.26%p" 같은 요인 수치 나열은 하지 마세요 — 부호(유리/불리) 판단에만
     쓰고, 본문에는 초과/하회 폭만 숫자로 적습니다."""

BLOCK_EXTRA_RULES[('07G07', '계획')] = """   ★ 서술 순서 (사용자 확정본 구조 — "무엇을 보고, 무엇을 할지"):
     1) 다음 달 운용의 **1순위 변수**를 먼저 못박습니다.
        (확정본: "금리와 유동성 변화에 우선적으로 대응할 계획입니다")
     2) 당월에 이미 취한 조치 → 다음 달 **조건부 계획**. 조건을 구체적으로:
        "장기금리가 추가 상승할 경우 … 듀레이션을 점진적으로 확대" 처럼 **트리거와
        행동을 한 쌍**으로 씁니다.
     3) 주식은 유지할 근거와, 무엇을 보고 조정할지(지표·이벤트)를 함께.
   ★ "현재의 포트폴리오를 유지하되 괴리가 확대되면 리밸런싱하겠습니다" 같은 **범용
     문장은 금지**입니다 — 어느 펀드에나 붙는 말이라 정보가 없습니다. 반드시 이번 달
     매크로 판단과 연결된 구체적 계획을 쓰세요.
   ★ 모범 예시(2026-08 확정본):
     "9월 운용은 금리와 유동성 변화에 우선적으로 대응할 계획입니다. 8월 중 장기국채
      비중을 소폭 확대했으며, 9월에도 장기금리가 추가 상승할 경우 장기채의 가격
      매력을 활용해 듀레이션을 점진적으로 확대할 계획입니다." """
...
# 4JM12 펀드성과 — **원인**을 쓰게 한다 (2026-08-07 사용자 지시).
# ⚠ 1차 산출물이 "자산배분효과는 …+0.11%p로 긍정적, 종목선택효과가 -2.36%p로 부진"
#   처럼 **요인 이름과 수치만 나열**하고 왜 그랬는지가 하나도 없었다. 발송본은 요인
#   용어를 아예 쓰지 않고 포지션·운용 행위로 설명한다.
BLOCK_EXTRA_RULES[('4JM12', '펀드성과')] = (
    '   ★ 이 블록의 핵심 (반드시 지킬 것):\n'
    '     · 1문장: 펀드 수익률 · BM 수익률 · 초과폭(bp).\n'
    '     · 이어서 **무엇이 상대성과를 갈랐는지 원인으로** 쓰세요 — 어떤 포지션·운용\n'
    '       결정이 유리했고 무엇이 불리했는지, **그 이유까지**.\n'
    '     · ★ **자산배분 효과와 종목선택 효과를 섞지 마세요.** 같은 자산군이라도 두\n'
    '       층위는 부호가 반대일 수 있습니다 — 요인표를 보고 어느 쪽인지 밝히세요.\n'
    '       (2026-07 실측: 해외주식은 **비중(UW)은 유리, 종목선택은 불리**)\n'
    '       "해외주식 포지션이 긍정적이었으나 해외주식 포지션은 부정적" 처럼 같은\n'
    '       이름을 양쪽에 쓰면 실패입니다. "저비중 대응은 유리했으나 성장주 종목\n'
    '       구성은 불리" 처럼 **무엇이** 유리/불리였는지 구분해 쓰세요.\n'
    '     · ★ 환 서술은 **BM 대비 환노출로만** 쓰세요 (2026-08-07 사용자 확정).\n'
    '       이 문단은 상대성과를 다루므로 절대 손익("환효과가 몇 %p 손실")은 쓰지\n'
    '       마세요 — 절대 기여와 BM 대비 방향은 반대일 수 있어 섞으면 인과가 꼬입니다.\n'
    '       (2026-07 실측: 절대로는 환효과 손실이지만 BM 대비 환노출이 낮아 **상대성과\n'
    '        에는 유리**했다. "축소된 환 익스포저가 환효과 손실로 이어지며" 는 오답.)\n'
    '     · ★ **"자산배분효과 +0.11%p, 종목선택효과 -2.36%p" 같은 요인 수치 나열 금지.**\n'
    '       Brinson 요인표는 **부호(유리/불리) 판단 근거로만** 쓰고 본문에 옮기지 마세요.\n'
    '     · 자산군별 기여도(-4.33% 등)도 나열하지 마세요. 원인 서술에 필요한 만큼만.\n'
    '     · 모범 예시 — 요인 용어 없이 포지션과 사유로 설명합니다:\n'
    '       "최근 1개월간 펀드는 +0.69%의 수익률, BM은 -0.15%의 수익률을 기록하여 BM '
...
BLOCK_EXTRA_RULES[('4JM12', '운용경과')] = (
    ...
    '        **순매도로 찍힌 자산군은 하나도 빠뜨리지 말고** 재원으로 쓰세요.\n'
    '        (2026-07 실측: 국내채권 외에 **해외주식 순매도**도 재원인데 누락됐다.)\n'
    '        금액은 방향 판단에만 쓰고 본문에는 적지 않습니다 — 아래 표기 규칙 참조.\n'
    '        ★ 만기대(초장기물·중기물·단기물) 변경은 **금리 전망과 묶어** 쓰세요.\n'
    '          단순히 "편출/축소" 로 끝내지 말고 왜 그 만기대를 줄였는지 — 예: 고금리\n'
    '          장기화 국면에서 금리 위험이 큰 초장기물을 정리하고 단기물로 전환.\n'
    '     ③ **환 포지션** — 달러선물 포지션 변화와 **그 목적**. 제공된 환헤지 비율\n'
    '        표(전월→당월, BM 기준)를 근거로 목적을 밝히세요. 방향만 쓰면 실패입니다.\n'
    '        ★ **반드시 BM 대비로 서술**하세요. 헤지비율이 BM 보다 높으면 환 익스포저는\n'
    '          BM 대비 **작은** 것입니다 — 전월 대비 헤지를 줄였더라도 여전히 BM 보다\n'
    '          많이 헤지 중이면 "환 익스포저 확대" 로 끝내면 **틀립니다**.\n'
    '          (2026-07 실측: 헤지비율 110.5%→55.8%, BM 50.0% → 오버헤지는 해소했지만\n'
    '           환 익스포저는 여전히 BM 대비 축소 상태)\n'
    '   ★ 표기 규칙 (반드시 지킬 것):\n'
    ...
    '             스트립(채) · 머니마켓 · KOSPI200 · TMF28 처럼 **상품을 특정하는\n'
    '             축약도 금지**입니다.\n'
    '       바꿔 쓰기: 30년스트립·스트립채 → **초장기물** / 국고채10년 → **중기물** /\n'
    '             머니마켓 → **단기물·현금성 자산** / KOSPI200 → **국내주식**\n'
    '       허용: 국내주식 · 해외주식 · 국내채권 · 해외채권 · 대체 · 현금, 그리고\n'
    '             미국 성장주 · 미국 가치주 · 선진국 주식 같은 **스타일·지역 구분**,\n'
    '             초장기물 · 중기물 · 듀레이션 · 환헤지 같은 **만기대·포지션 표현**.\n'
    '     · ★ **거래 볼륨(매매금액)을 쓰지 마세요.** "17.4억을 순매수", "21.4억 순매도"\n'
    '       처럼 억 단위 금액을 적지 말고 **행위만** 쓰세요 — 순매수/순매도/확대/축소/\n'
    '       편출/신규 편입. (2026-08-06 사용자 지시)\n'
    '     · 다만 **시장 레벨·환율 레인지 숫자는 괜찮습니다** — "KOSPI 5,200선",\n'
    '       "1,440원~1,520원" 은 발송본이 실제로 쓰는 표기입니다.\n'
    '     · 모범 예시 — **2026-07 사용자 확정본**. 문체와 인과 밀도의 기준이며 '
    '내용은 재사용하지 마세요:\n'
    '       "국내주식의 극단적 급락 이후 반등 국면에 대응하고자 국내주식을 신규 '
    '편입하였습니다. 재원은 고금리 장기화 국면에서 금리 위험이 큰 초장기물을 전량 '
    '편출하고 중기물을 축소해 마련하였으며, 일부는 단기물·현금성 자산으로 옮겨 '
    '듀레이션을 짧게 가져가며 재투자 여력을 확보하였습니다. 환헤지는 원화 강세를 '
    '반영해 과도했던 오버헤지를 해소하되, 되돌림 위험을 감안해 환 익스포저는 BM 대비 '
    '축소된 수준을 유지하였습니다."'
)

BLOCK_EXTRA_RULES[('4JM12', '운용계획')] = (
    '   ★ 표현 규칙 (반드시 지킬 것):\n'
    '     · **비중·수익률·금액을 쓰지 마세요** (시점 표기는 괜찮습니다).\n'
    '     · **개별 종목명·티커 금지** (ACE 200 / ACE 미국대형성장주액티브 등). '
    '자산군이나 자산 유형(미국 성장주·국고채) 표기는 괜찮습니다.\n'
    '     · **정도부사 금지** — 대폭·크게·급격히·상당히·큰 폭으로·대거.\n'
    '     · 방향(유지/확대/축소)과 그 판단 근거를 함께 쓰세요. 나열만 하면 실패입니다.\n'
    '     · 모범 예시 — 문체 기준이며 내용은 재사용하지 마세요:\n'
    '       "주식 및 채권은 중립수준을 유지 예정. 스타일 측면에서 채권 듀레이션과 '
...
# 판단 근거가 들어갔는지 보는 약한 신호. 1차 실패본("…순매수하여 비중을 확대하는
# 리밸런싱을 실행하였으며…")에는 이런 표현이 하나도 없었고, 확정본에는 "판단해" 가 있다.
_PLAN_REASONING = ('판단', '때문', '감안', '고려', '우려', '기대', '전망',
                   '따라', '평가', '보아', '보고')

# 거래 볼륨(매매금액) — "17.4억을 순매수" 같은 표기. 2026-08-06 사용자 지시로
# 4JM12 운용경과에서 금지한다. 행위(순매수/축소/편출)만 쓰고 금액은 적지 않는다.
# ⚠ 숫자 전체를 막으면 안 된다 — 시장 레벨("KOSPI 5,200선")과 환율 레인지
#   ("1,440원~1,520원")는 발송본이 실제로 쓰는 표기다. **억 단위만** 잡는다.
_TRADE_VOLUME_RE = re.compile(r'\d[\d,]*(?:\.\d+)?\s*억')

# 정량 표기 — 비중·수익률·금액·bp. 운용계획 블록에서 금지한다.
# ⚠ **숫자 전체를 막으면 안 된다.** 2026-07 2JM23 발송본이 "미국 중간선거가 있는
#   11월 이후에는…" 처럼 **시점**을 쓴다. 단위가 붙은 정량만 잡는다.
_QUANT_RE = re.compile(r'\d[\d,]*(?:\.\d+)?\s*(?:%p|%|억|만원|원|배|bps|bp)')

# Brinson 요인 수치 나열 — "자산배분효과 +0.11%p" 처럼 **이름+수치만** 옮긴 문장.
# 원인 서술 없이 이것만 있으면 읽는 사람이 얻는 게 없다(2026-08-07 사용자 지적).
_FACTOR_LISTING_RE = re.compile(
    r'(자산배분효과|종목선택효과|배분효과|선택효과)[^.]{0,24}?[-+−]?\d+(?:\.\d+)?\s*%p')
...
_BLOCK_SHAPE = {
    # ⚠ 2026-07 **실제 발송본**은 4문장 222자다 — ①판단→실행 ②재원 뒤에 향후 계획
    #   2문장이 더 붙는다. 종전 (2문장 110~170자)은 발송본을 반려시켰다(실측).
    ('2JM23', '계획'):
        {'sent': (2, 4), 'chars': (110, 250), 'reason': True,
         'no_quant': True, 'no_adv': True},
    ('4JM12', '운용계획'):
        {'sent': (1, 4), 'chars': (90, 250), 'reason': False,
         'no_quant': True, 'no_adv': True},
    # 목적→재원→환포지션 3단이라 2JM23 블록보다 길다.
    # 거래 볼륨(억)만 금지하고 시장 레벨·환율 레인지 숫자는 허용한다.
    ('4JM12', '운용경과'):
        {'sent': (2, 3), 'chars': (150, 270), 'reason': False,
         'no_quant': False, 'no_adv': False, 'no_volume': True},
    # 펀드성과 — 수익률·bp 는 써야 하므로 숫자 허용, 요인 수치 나열만 금지
    ('4JM12', '펀드성과'):
        {'sent': (2, 3), 'chars': (150, 280), 'reason': False,
         'no_quant': False, 'no_adv': False, 'no_factor_listing': True},
    # 07G07(포맷 K) — 2026-08 사용자 확정본 실측으로 잡았다.
    #   매매 2문장 137자 · 성과 2문장 158자 · 계획 3문장 249자
    # ⚠ 성과는 **환율 서술이 더 붙으므로** 상한을 넓혀 둔다(2026-09-01 사용자 지시).
    #   요인 이름("자산배분 성과", "종목선택 효과")은 확정본이 쓰므로 막지 않고,
    #   요인 **수치** 나열만 no_factor_listing 으로 막는다.
    ('07G07', '매매'):
        {'sent': (2, 3), 'chars': (110, 220), 'reason': False,
         'no_quant': True, 'no_adv': False},
    ('07G07', '성과'):
        {'sent': (2, 5), 'chars': (140, 380), 'reason': False,
         'no_quant': False, 'no_adv': False, 'no_factor_listing': True},
    # 계획은 정량 표기를 막지 않는다 — 확정본이 "장기금리가 추가 상승할 경우" 처럼
    # 금리 레벨을 근거로 쓰고, 실제 생성에서도 기준금리 3.00% 인용이 나왔다(정상).
    ('07G07', '계획'):
        {'sent': (2, 4), 'chars': (150, 320), 'reason': False,
         'no_quant': False, 'no_adv': False},
}
...
def check_block_rules(fund_code: str, block: str, text: str) -> list[str]:
    ...
    if (fund_code, block) not in BLOCK_EXTRA_RULES or not text:
        return []
    import re as _re
    spec = _BLOCK_SHAPE.get((fund_code, block))
    if spec is None:        # 규칙만 있고 형태 미등록 — 검사 생략(생성은 살린다)
        return []
    out = []

    if spec['no_quant']:
        q = _QUANT_RE.findall(text)
        if q:
            out.append(f'정량 표기(비중·수익률·금액): {", ".join(sorted(set(q))[:6])}')
    if spec['no_adv']:
        advs = [a for a in _PLAN_ADVERBS if a in text]
        if advs:
            out.append(f'정도부사 사용: {", ".join(advs)}')
    if spec.get('no_volume'):
        vols = _TRADE_VOLUME_RE.findall(text)
        if vols:
            out.append(f'거래 볼륨 표기: {", ".join(sorted(set(vols))[:5])}')
    if spec.get('no_factor_listing'):
        f = _FACTOR_LISTING_RE.findall(text)
        if f:
            out.append(f'Brinson 요인 수치 나열: {", ".join(sorted(set(f))[:3])} '
                       f'— 원인 서술로 바꾸세요')
    ...
    if not s_lo <= n_sent <= s_hi:
        want = f'{s_lo}문장' if s_lo == s_hi else f'{s_lo}~{s_hi}문장'
...
def assemble_seeded_comment(fmt: str, market: str, outlook: str,
    ...
    if fmt == 'E':
        # DB생명 6슬롯. 라벨 표기는 전월 발송본 그대로 — '운용계획:' 만 콜론 앞 공백이 없다.
        return (
            '운용경과\n'
            f'시장 동향 : {market}\n'
            f'운용 경과 : {blocks["운용경과"]}\n'
...
def build_report_prompt(fund_code, year, quarter, data_ctx, inputs,
    ...
    if amc:
        ...
        for it in amc[:6]:
            ...
            paths = " / ".join((it or {}).get('causal_paths') or [])
            amc_lines.append(f"- [{ac}] 과거: {past}")
            if drivers:
                amc_lines.append(f"  drivers: {drivers}")
            # ★ causal_paths 가 '왜 그렇게 움직였는가' 사슬인데 종전엔 렌더가 빠져
            #   프롬프트에 원인이 전달되지 않았다 (2026-09-01).
            if paths:
                amc_lines.append(f"  인과경로: {paths}")
            if outlook:
    ...
    # 환(FX) 사실 — 코멘트 PA 는 fx_split=False 라 환이 각 해외 자산군에 접혀 있어
    # LLM 이 **환 숫자를 아예 못 본다**. 2026-08 07G07 실측: 원화가 한 달 4.0% 절상
    # (DXY 는 -0.5%)해 환 기여가 -0.70%p 였는데 코멘트에 환 언급이 한 줄도 없었다
    # (2026-09-01 사용자 지적). `fund_comment_service._build_fx_facts` 가 같은 창을
    # fx_split=True 로 한 번 더 돌려 환 행만 뽑아 준다.
    # ⚠ 절대 기여와 BM 대비 효과는 **부호가 반대일 수 있어** 따로 준다.
    fx_text = ''
    fxf = data_ctx.get('fx_facts') or {}
    if fxf.get('fx_contrib') is not None:
        _fx = [
            '  환이 펀드 수익률에 미친 크기: {:+.2f}%p '.format(fxf['fx_contrib'])
            + '(위 자산군 표는 FX 포함 기준이라 이 몫이 해외 자산군 수익률에 '
              '이미 들어가 있습니다 — 따로 더하지 마세요)',
            '  환 자산배분 효과(BM 대비): {:+.2f}%p'.format(fxf.get('fx_alloc', 0.0)),
            '  환노출 비중: {:.1f}% · 환헤지비율: {:.0f}%'.format(
                fxf.get('fx_weight', 0.0), fxf.get('hedge_ratio', 0.0)),
        ]
        if fxf.get('usdkrw_chg') is not None:
            _k = fxf['usdkrw_chg']
            _line = '  달러/원 {:+.2f}% ({})'.format(
                _k, '원화 약세' if _k > 0 else '원화 강세')
            if fxf.get('dxy_chg') is not None:
                _line += ' · 달러지수(DXY) {:+.2f}%'.format(fxf['dxy_chg'])
                _line += ' → 원화 변동이 달러지수보다 크면 원화 고유 요인입니다.'
            _fx.append(_line)
        fx_text = ('\n\n## 환(FX) 사실 — 자산군 수익률은 **FX 포함 기준**이라 환이 각 해외 '
                   '자산군에 이미 접혀 있습니다.\n## 아래는 환율 자체를 서술하기 위한 '
                   '참고 수치입니다.\n' + '\n'.join(_fx))
...
## 작성 규칙 — 수치 배치 (발송본 관행, 반드시 지킬 것)
8-1. **시장 서술은 정성, 펀드 성과는 정량**. 시장동향 문단에서는 퍼센트를 쓰지 말고
   방향과 레벨로 서술하세요. 예: "유가가 전쟁 이전 수준까지 하락", "달러/원이 1,550원
   부근까지 상승", "월간 기준 소폭 상승 마감". 반대로 펀드 수익률·BM 대비 초과·자산군별
   기여도는 제공된 수치를 정확히 인용하세요.
8-2. **월중 일자별 등락률 금지**. "7월 13일 -8.95%" 같은 특정일 수치를 쓰지 마세요.
   사건 자체는 수치 없이 서술합니다. 예: "서킷브레이커가 발동되는 등 극단적 변동성이
   교차하였습니다". 정량 수치는 **월간(기간) 등락**에 한합니다.
8-3. **같은 지수의 월간 등락은 1회만** 인용하세요. 여러 지수를 나열하거나 같은 수치를
   반복하지 마세요. 자산군별로 대표 지수 하나면 충분합니다.
8-4. **보유 자산군만 서술** (2026-08-03 사용자 지시): 시장동향·운용경과·전망 전 구간에서
   해당 기간에 펀드가 **실제로 보유한 자산군만** 다루세요. 아래 "펀드 보유 자산 비중"과
   "PA 자산군별 기여도"에 나타나지 않는 자산군은 시장 서술에서도 언급하지 마세요.
   예컨대 금·리츠를 보유하지 않는 펀드라면 대체투자 문장을 아예 쓰지 않습니다.
   보유하지 않은 자산군을 채우려고 일반론을 덧붙이는 것이 가장 나쁜 서술입니다.
8-5. **섹션 간 중복 금지** (2026-08-05 사용자 지시): 같은 내용을 두 섹션에 쓰지 마세요.
   - **전망 섹션은 시장 전망만** 씁니다. 운용철학·목표변동성·리밸런싱 상투구
     ("경기국면모델에 기반한 목표변동성을 유지하면서 …", "목표 포트폴리오와의 괴리가
     과도할 경우 리밸런싱을 통해 재조정할 계획입니다" 등)를 전망 섹션 끝에 붙이지 마세요.
     이 내용은 매니저 코멘트(또는 포지션 섹션)에서 이미 다룹니다.
...
## 운용역 판단 (반드시 반영)
{input_text}{constraint_text}{past_sample}{narrative_text}{pattern_text}{fx_text}{evidence_block}
