...
4. **시장 서술은 정성적으로**. 퍼센트 등락률을 쓰지 말고 방향과 레벨로 쓰세요.
   ...
   ⚠ 실측 — 시드가 8월말 달러/원을 "1,530원대"로 썼습니다(실제 1,368.60).
   레벨의 정본은 DB 입니다.
   ★ **제공되는 고저는 종가 기준**입니다. 사내 DB 에 지수 장중 고저가 없으므로
   ("장중 5,200선까지" 같은) **장중 레벨 숫자는 이 블록으로 뒷받침되지 않습니다** —
   장중 표현에 이 숫자를 그대로 붙이지 마세요.
