...
def get_master_data_hash():
    """마스터 데이터 원본의 해시 계산 (버전 관리용)"""
    import hashlib
    return hashlib.md5(MASTER_DATA_RAW.encode('utf-8')).hexdigest()
...
def create_initial_master():
    ...
    # 해시 저장 (버전 관리)
    data_hash = get_master_data_hash()
    with open(MASTER_FILE, 'wb') as f:
        pickle.dump({'data': df, 'hash': data_hash}, f)
    print(f"[✓] 초기 마스터 테이블 생성 완료: {len(df)}개 종목 (hash: {data_hash[:8]}...)")
...
def load_master_mapping():
    ...
    마스터 분류 테이블 로드
    - 파일이 없으면 새로 생성
    - 원본 데이터(MASTER_DATA_RAW)가 변경되면 강제 재생성
    """
    current_hash = get_master_data_hash()
    ...
    try:
        ...
        # 새 형식: {'data': DataFrame, 'hash': str}
        if isinstance(loaded, dict) and 'hash' in loaded:
            saved_hash = loaded['hash']
            if saved_hash != current_hash:
                print(f"[WARNING] 마스터 원본 데이터 변경 감지! 재생성...")
                print(f"  기존 hash: {saved_hash[:8]}... → 새 hash: {current_hash[:8]}...")
                return create_initial_master()
            return loaded['data']

        # 구 형식: DataFrame 직접 저장 → 새 형식으로 마이그레이션
        print(f"[INFO] 구 형식 마스터 파일 감지. 새 형식으로 마이그레이션...")
        df = loaded
        with open(MASTER_FILE, 'wb') as f:
            pickle.dump({'data': df, 'hash': current_hash}, f)
        return df

    except Exception as e:
        print(f"[ERROR] 마스터 파일 로드 실패: {e}. 재생성...")
        return create_initial_master()
