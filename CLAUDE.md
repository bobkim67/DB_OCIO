...
|---|---|---|
| 1 표지 | 기준월 | — |
| 2 운용현황 | 표② 펀드수익률 · ③ 코멘트 | 연환산 변동성 · KOSPI200 · MSCI ACWI · ① 그래프+헤더 |
| 3 자산배분 | 당월말·전월말 비중 · 기여도 · 소계/총계 | **TAA 열** |
| 4 종목·전망 | 표⑤ 전체 · ⑥ 코멘트 | 향후 관리방안 열 |

### 검증 (2026-06 발송본 대조)

표④ 9행 **전 행 일치** · 표⑤ **전 항목 일치** · 표② 1M·3M·6M·YTD **정확 일치**.
...
- **보수 환원** = BOS3203 컴포넌트 합 **7.225 → 연 0.7225%** 일할 복리.
  ⚠ `load_fund_meta().fee_bp` 는 컴포넌트 하나(0.16)만 집어온다 — **버그**.
  Overview 화면 '총보수'도 같은 값을 쓴다.
...
- 표④ 행 매칭은 **라벨 기준**. 템플릿 채권 행 순서가 `SHINHAN_PPT_SKELETON` 과
  달라 인덱스로 쓰면 값이 어긋난다.
