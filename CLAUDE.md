...
## KB국민은행(07G07) 월간 **워드** — Word COM 폐기 → 엑셀 붙여넣기 (2026-08-07)

`tools/kb_monthly_excel.py` (표 · 서술 2시트). 신한 PPT 와 **같은 전환**이다 —
발송본 워드는 그대로 열어 두고 엑셀에서 블록을 복사해 표에 붙여넣는다.

⚠ `tools/kb_monthly_report.py` 의 **`collect()` 는 계속 쓴다**(수치 산출 단일 소스).
폐기 대상은 `build_docx`(Word COM 치환)뿐 — 한쪽만 고치면 같은 달 보고서 안에서
값이 갈린다. 07G07 은 매월 **워드 + FactSheet 엑셀** 2종이 나간다 — FactSheet 는
2026-09-01 에 **같은 워크북의 3번째 시트**로 붙었다(아래 별도 절).

### 붙여넣기 격자 (발송본과 1:1)

| 워드 | 격자 | 엑셀 `표` 시트 |
|---|---|---|
| 표3 성과요인분해 | 9행 × 5열 | `A3:E11` |
| 표4 운용수익률 | 값 = 3~7열 | `C17:G17` |
| 표5 투자비중 | 값 = 3~10열 | `C23:J23`(SAA) · `C27:J27`(전월말) |

- ★ **행 순서가 곧 정확도** — `FACTOR_ROWS`(주식/채권/FX/유동성 및 비용)·
  `WEIGHT_KEYS`. COM 치환기는 셀 좌표로 찍어 순서 오류가 안 보였지만 블록
  복사는 값이 엉뚱한 행에 붙는다(신한 채권 4행과 같은 함정). 테스트가 고정.
- 값은 **문자열**로 쓴다 — 숫자+표시형식이면 '값만 붙여넣기'에서 원본 숫자가 튄다.

### 확정된 기준 (2026-08-07 사용자)

- **표3 헤더 = 기준가 기준**(펀드 = 07G07 기준가 1개월). 이러면 **요인 합계 = 헤더
  초과**로 정합한다. ⚠ 2026-05 발송본은 헤더에 마스터 PA(+7.39/+2.80/+4.58)를 쓰고
  요인 합계는 기준가 초과(+3.76)라 표 안에서 두 기준이 섞여 있었다 →
  `_check` 가 0.05%p 초과 시 경고한다.
- **설정 후\* = 앵커 SI**(2022-01-03 기준 절대수익률). 2026-07 = **+30.15**.
  ⚠ 구 발송본은 이 칸에 **설정후 초과**를 넣었다 — 실측: 2026-05 초과 15.4466 =
  발송본 +15.44 / 2026-06 초과 11.2953 vs 발송본 +11.26. 이제 절대값을 쓴다.
- **성과요인분해 = FX 분리 · 방법1**(FX 행이 따로 있다). FactSheet 는 FX 포함 ·
  방법3 이라 **서로 다르다** — 섞지 말 것.
- **유동성 비중 = 잔여(100 − Σ)**. 현금·예금은 MA000410 에 라인이 없다.

### 서술 시트 — 3층으로 갈라 쓴다

| 층 | 무엇 | 색 |
|---|---|---|
| 코드 산출 | 헤드라인·환기여수익률·BM대비 초과성과 | 파랑 |
| **운용역 확정** | `NARRATIVE_OVERRIDES[period][블록]` 원문 그대로 | 초록 ★확정 |
| LLM 초안 | 나머지 22블록 | 흰색 |

- 순수 수치 나열에 LLM 을 끼우면 오기만 생긴다(2JM23 성과 문장과 같은 규약).
- **override 는 프롬프트에도 '이미 확정된 문장'으로 넣는다** — 안 그러면 나머지
  블록이 반대 방향으로 흘러간다. 실측(2026-07): 확정한 "반도체 조정=저가매수"
  판단을 전망·계획 블록이 일관되게 따라왔다. 스켈레톤에서는 빼서 생성도 안 시킨다.
- `max_tokens` 잘림은 **채택하지 않는다** — 잘린 JSON 이 부분 파싱되면 뒤쪽 블록이
  조용히 빈다([[reference_debate_token_cap]] 과 같은 함정).

### 검증 (2026-05 발송본 대조)

- 요인 16셀 **±0.01%p** · 비중 유동성만 0.1%p(잔여 정의차)
- **절대성과 3값 정확 일치** — 주식 +7.88 / 채권 -2.12 / 환기여 +0.30
- 2026-07 산출: 표5 주식 39.9(=국내 21.4+해외 18.5)·채권 59.3 이 **승인 코멘트와 일치**

배선 = `_EXCEL_SPECS['07G07']`, **보고서 승인 시** 재생성 → `/report/excel`.
⚠ 승인 응답에 LLM 1회가 들어가 수십 초 걸린다.

---

## KB국민은행(07G07) **FactSheet** — 같은 워크북 3번째 시트 (2026-09-01)

FactSheet 빌더는 2026-08-04 커밋(`1edbd1e`)에 "미완"으로 남아 있었다 —
`collect()` 가 FactSheet 기준(FX 포함·방법3, 앵커 SI)까지 이미 내고 있었는데
**쓰기 층과 다운로드 배선만 없었다**. 다운로드는 펀드당 산출물 1개만 지원하므로
(`/report/excel` 1개 · 프론트 버튼 1개) 별도 스펙을 만들지 않고 **워드 엑셀과 같은
파일에 `FactSheet` 시트**로 얹었다 (2026-09-01 사용자 지시).

### 붙여넣기 격자 — 양식과 **셀 주소가 1:1**
...
| 운용수익률 | `A9:AD9` |
| 벤치마크수익률 | `A16:X16` |
| 벤치마크수익률(동일헤지 가정) | `A24:V24` |

- ⚠ **숫자로 쓴다** — 워드용 `표` 시트(문자열)와 규약이 **반대**다. 양식이 raw float
  을 담고 있어 문자열을 붙이면 칸이 텍스트가 되어 이후 계산이 깨진다.
- 미채움 칸은 **비워 둔다**(0 금지) — 0 은 '헤지 없음'처럼 유효한 값이라 산출 실패와
  구분되지 않는다.
- 양식은 DRM(`<DOCUMENT SAFER`) 이라 열어 둔 채 이 시트에서 블록만 복사한다.

### 확정된 기준 (2026-07 발송본 대조로 판별)

- **비중 = PA 비중 · 방법3**. 방법1 은 주식/채권으로 접혀 국내·해외 구분이 없다
  → 6분류(C3:H3)를 못 채운다. 실측 21.39/18.54/59.28/0.00 = 발송본 일치.
- **펀드듀레이션(Y9) = 채권듀레이션(반올림 1자리) × 국내채권 PA비중**.
  holdings 의 `duration_overall`(10.356)이 **아니다** — 그쪽은 보유 스냅샷 비중
  (58.55%)이라 비중 칸(59.28%)과 기준이 갈린다. 실측 10.49256 vs 발송본 10.49300.
- **수정샤프(X9·X16)는 일반 샤프가 아니다.** R `module_00_Function_v3.R:1619`:
  `if_else(ann-rf > 0, (ann-rf)/risk, (ann-rf)*risk)` — **초과가 음수면 나누지 않고
  곱한다**(변동성 큰 펀드가 덜 나빠 보이는 역전을 막으려고). 시스템 표(07G07,
  2026-08-31) 13개 기간 중 12개가 이 식으로 소수 9자리까지 재현된다(YTD 셀만 표기
  스케일 100배). `modules.data_loader.compute_adjusted_sharpe_ratio`.
  ★ 이 정의가 두 미스터리를 동시에 설명한다 — 2026-07 발송본 X9(+0.0166377)는 초과가
    양수라 나눗셈이어서 일반 샤프와 같았고, X16(-0.000466)은 BM 초과가 음수라 곱셈이라
    일반 샤프(-0.16)와 자릿수가 통째로 달랐다.
- **무위험 = ECOS CD(91일) 일할복리지수**(`data_loader._load_rf_index_ecos`).
  R `return_rf_index("010502000")` 재현 — `1000*cumprod(1+금리/100/365)`. 2026-09-01 에
  KIS CD Index(SCIP 194/33)에서 교체했다. 시스템 RF 행과 0.000bp 일치(종전 KIS 는 1M
  -19.6bp·YTD -1.20bp 어긋났다). ECOS 가 간헐적으로 죽으므로 `.cache/rf_ecos_cd91.csv`
  를 먼저 보고, 캐시도 없을 때만 KIS 로 폴백한다 — 소스가 바뀌면 같은 달 안에서 값이 갈린다.
- **연환산수익률·연율화위험은 시스템과 이미 일치**한다(2026-08 실측 0.0000%p·0.000000%p).
  연율화위험 = **단순** 주간수익률 sd × √52(v1). R 앱 기본값은 v2(로그)지만 실제 출력은 v1이다.
- **BM 18M/24M/30M·변동성·수정샤프**는 `_compute_bm_period_returns` 가 안 낸다.
  그 함수의 키 집합은 `test_overview_smoke.py` 가 고정(대시보드 계약)하므로 늘리지
  않고, 같은 BM 시계열에 펀드와 같은 주간수익률 규약을 적용해 `_bm_metrics` 에서
  따로 낸다. 설정후(O16)만 `build_period_returns` 의 SI 를 쓴다.
- **동일헤지 가정 BM(24행) = 일반 BM 복사**. 07G07 은 환헤지비율 0(무헤지)이라
  '동일수준 헤지' = 무헤지 → 같은 값이다. 7월 발송본도 B24:V24 = B16:V16 이었다.
  헤지비율이 0 이 아니면 `_fs_notes` 가 경고한다.

### 검증 (2026-07 발송본 54셀 대조) — 51셀 ±0.05%p 이내

재현 안 되는 3셀은 발송 전 확인 대상이고 `_fs_notes`·`_check` 가 경고로 띄운다:

| 셀 | 발송본 | 재현 | 메모 |
|---|---|---|---|
| `R9` 해외주식 연초이후 | 11.4156 | 10.12 | **1.3%p** — YTD 기초일(12/31·1/1·1/2)을 바꿔도 10.12~10.15 라 창 문제가 아니다. 같은 칸 BM(`R16`)은 일치 → AP 쪽 |
| `X16` BM 수정샤프 | -0.000466 | -0.00048 | 음수 분기(초과×위험)로 자릿수는 해결. 남는 미세차는 아래 §BM 규약 차이 |
| `L16` BM 18M | 7.1439 | 7.0793 | -0.065 — `K16`/`M16`/`N16`(-0.02~-0.04)과 같은 BM 리비전 드리프트 |

### ★ BM 변동성·수정샤프(W16·X16)는 시스템과 구조적으로 다르다 — 오차가 아니다 (2026-09-02)

`결과7.수정샤프비율` 등 시스템 백테스트 산출과 대조한 결과:

| 칸 | 내 빌더 | 시스템 | |
|---|---|---|---|
| W9·X9 (펀드) | 16.790429226803 / -0.000242956638 | 동일 | **소수 12자리 일치** |
| W16 (BM 변동성) | 5.898777436789 | 5.887685209998 | 0.188% |
| X16 (BM 수정샤프) | -0.000574856882 | -0.000564654165 | 1.807% |

창·산식은 같다 — YTD 주간 34개(2026-01-12~08-31)로 양쪽 다 정확 재현된다.
갈리는 건 **BM 시계열 하나**이고, 원인은 **해외 레그 T-1 을 세는 달력**이다:

| | 7/20(월)에 쓰는 MSCI ACWI |
|---|---|
| 이 빌더 = **DT 전산 BM 지수** | **7/17** 종가 1,108.73 (캘린더 T-1) |
| 시스템 = **컴포넌트 합성**(`biz_day_adj=-1`) | **7/16** 종가 1,121.69 (한국 영업일 T-1) |

7/17 은 한국 휴장인데 해외는 열려 ACWI 가 **-1.1553%** 빠졌다.
예측 -1.1553 × 34%(ACWI 비중) = **-0.393%**, 실측 지수 갭 **-0.383%**. 5/1 휴장도
같은 부호로 재현된다(ACWI +0.2652% → 예측 +0.090% / 실측 +0.111%).

- 화요일에 양쪽이 같은 종가로 복귀하므로 **다음 주에 상쇄**된다. 2주 복리는
  0.982778 vs 0.982781 — **누적수익률은 사실상 동일하고 주간 분산만 갈린다.**
- 2026-08 실측: 34주 중 **31주 완전 일치**, 어긋난 건 휴장 낀 5월·7월 쌍(±11bp·±38bp)뿐.
- ⚠ **부분 수정 금지.** 최종 갭 0.011%p 는 5월(+0.05%p)과 7월(-0.05%p)이 우연히
  상쇄된 값이라, 한쪽만 고치면 **갭이 5배로 벌어진다**.
- ★ **DT 전산지수를 유지한다**(2026-09-02 사용자 확정). 따라서 W16·X16 은
  **시스템 값으로 덮어쓰는 것이 정본**이며, 빌더가 `_fs_notes` 로 그렇게 안내한다.
  X9(펀드)는 빌더 값이 시스템과 일치하므로 그대로 쓴다.
- 소형 잔여(4/13·4/20 ±0.23bp 등)는 휴장이 없는 주라 이 메커니즘으로 설명되지
  않는다 — 0.5bp 이하라 방치한다.

### 환헤지비율 AB9~AD9 — 해외채권은 상수 100

`AA9`(전체)를 세 칸에 복사하던 것을 고쳤다(07G07 은 전부 0 이라 우연히 맞았다).
**해외채권은 편입하면 전액 환헤지**가 운용 규약이라 **비중과 무관하게 100** 이다
(`FS_BOND_HEDGE_PCT`, 2026-09-02 사용자 확정). 해외주식·대체는 전체 비율을 쓴다.

`H9`·`V9`(유동성 수익률)은 브린슨에 손익 라인이 없어 0 이다 — 7월 발송본의
0.03058·0.035 는 수기 기재값이다.
...
## 성과분석 탭 — BM 해외주식 ACWI 4분할 · 좌우 배치 (2026-08-24~25)

### ★ ACWI 주식 4분할 — 소스를 틀리기 쉽다

BM 의 해외주식 레그(MSCI ACWI)를 **스타일·지역 4버킷**으로 쪼개 표0(BM 구성)
종목펼치기 하위에 보여준다. 사내 SCIP `/reports/bm-wts` 와 **같은 매핑**을 쓴다
(`EQBM_MAPPING`, 2026-08-24 사용자 제공). 키는 SCIP `back_dataset.id`:

| dataset_id | 버킷 | 지수 | 비고 |
|---|---|---|---|
| 250 | `growth` | MSCI USA Growth (105825) | **표준**, IMI 아님 |
| 251 | `value` | MSCI USA Value (105826) | |
| 252 | `dm_int` | MSCI World ex-USA (991000) | |
| 231 | `em` | MSCI EM (891800) | |
| 239 / 278 | (채권) | BBG Global Agg ex USD / US Agg | 주식 분해엔 미사용 |

비중 소스 = **`dataseries_id=36 'FG Market Value'`**(시가총액). 2004-09-30 ~ 현재
일별 5,712건, 영업일 누락 0.

★ **근사가 아니라 정확 분해다.** MSCI 스타일 지수가 모지수 시총을 분할하므로
`USA Growth + USA Value + World ex-USA = WORLD`, `+ EM = ACWI` 가 성립한다.
190영업일 전수 검증 오차 **0.00000%** (WORLD 항등식 · EM 은 `dt.MI_DJISU` 891800 대조).
→ 4행 합이 ACWI 목표비중과 어긋나지 않아 잔차 행이 필요 없다.

⚠ **`solution.roboadvisorAPI_equitybenchmarkweights` 를 쓰지 말 것.** 이름·키가
(`EQ_DM_US_Growth`, `EQ_EM` …) 똑같이 생겼지만 **로보어드바이저 전용 벤치마크**다.
같은 FG Market Value 에서 파생돼 성장/가치는 소수 둘째 자리까지 일치하는데
**DM_INT/EM 경계가 달라** EM 이 9.35% 로 나온다(2026-08-21 정상값 11.87%).
실제로 이걸 소스로 오인해 한 번 틀렸다.

⚠ `dt.MI_CTY_W`(국가비중)는 **2025-02-07 ~ 2026-08-16 약 18개월 공백**이 있어
일별 시계열로 못 쓴다. `MI_DJISU.adj_mkt_cap_eod00d_usd` 는 공백 없음.

### 표시 규약

- **표시 전용.** Brinson 수식·BM 총수익률·초과성과에 일절 관여하지 않는다.
  4개 지수를 실제 BM 컴포넌트로 쪼개면 합성 수익률이 ACWI 단일 지수와 달라져
  **BM 총수익률이 +0.43%p 움직인다**(실측 08K88 YTD 18.10% → 18.53%). 그래서
  표1(기여수익률)은 **ACWI 한 행 유지**.
- **레그가 여러 개면 합쳐서 4행** + 헤지 구성은 하단 주석
  (4JM12 = `환노출 22.5% + 환헤지 22.5%`, 합계 45.0%). 2026-08-24 사용자 지정.
- 반올림 잔차는 최대 행에 흡수 — 안 하면 4행 합이 부모 행과 어긋난다
  (실측 4JM12 45.01 vs 45.00).
- 기준일 = **기말 스냅샷**(표0 AP 측이 기말 보유라 기준 일치). 주석에 시총 기준일 표기.
- 레그 판정은 **지수명에 'ACWI' 포함** 기준. 자산군 하드코딩(`== "해외주식"`)이면
  방법1·2 에서 ACWI 가 `주식`으로 매핑돼 분해가 통째로 사라진다.

### AP 순서 정렬 (2026-08-25 사용자 지정)

BM 4행을 **AP 보유 표시 순서**(비중 내림차순)에 맞춘다. 08K88 은 AP 가
성장·가치·EM·선진 순인데 BM 기본 순서가 성장·가치·선진·EM 이라 좌우 짝이 어긋났다.

- 버킷 판정 = `config/taa_classification.py` 의 **자산군_소**
  (`미국 성장주`/`미국 가치주`/`선진국 주식`/`신흥국 주식`) → ISIN 기준. 이름 추측 금지.
- 한 버킷에 종목이 여럿인 펀드(4JM12 YTD = 해외주식 8종목, 성장 3·가치 2)는
  **버킷 첫 등장 순서**를 쓴다. AP 에 없는 버킷은 기본 순서로 뒤에 붙인다.
- 행에 안정 키 `bucket`(growth/value/dm_int/em)을 실어 표시명이 바뀌어도 안 깨진다.
### 완전 정렬 — 빈 행 슬로팅 (2026-08-25)

단순 순서 정렬만으로는 **한 버킷에 종목이 여럿인 펀드에서 밀린다.** 4JM12 YTD 의 AP 는
성장·가치·선진·**가치**·EM·성장·… 순이라 EM 의 첫 등장이 5번째인데 BM 은 4행뿐이다.
→ 각 세부지수를 **같은 버킷의 첫 AP 종목과 같은 줄**에 놓고 사이를 빈 행으로 채운다.

- AP 행에 버킷 키를 하향: `BrinsonApCompItemDTO.bucket`(ISIN→`_bucket_by_isin`) ·
  `BrinsonSecContribDTO.bucket`(**ISIN 이 없어 종목명**→`_bucket_by_name`, 기말 미보유 종목용).
- 슬로팅은 프론트에서 — 화면 AP 목록(기말 스냅샷 + 기말 미보유 0% 행)을 만드는 곳이라
  목록 순서가 바뀌어도 안 깨진다. 백엔드가 인덱스를 계산하면 그 결합이 끊어진다.
- AP 에 없는 버킷은 뒤에 이어 붙인다. 빈 슬롯은 `label === ""` 로 판별해 셀을 비운다
  (`fmtWeight` 를 그대로 태우면 `0.0%` 가 찍힌다).
- 검증 4JM12: 성장↔성장주액티브 / 가치↔가치주액티브 / 선진↔DEVELOPED / (빈행)↔VANGUARD
  VALUE / **EM↔EMERGING** — 4행 전부 짝이 맞는다. 08K88(4:4)은 빈 행 없이 그대로.

### 표0·표1 좌우 배치 교대 (2026-08-25 사용자 지정)

`좌=BM / 우=AP` → **`좌=AP / 우=BM`**. 차이(표0)·초과기여(표1)는 각 우측 테이블의
마지막 열로 옮겨 **화면 최우측 유지**. 시작일 토글 모드에서는 우측이 기초일 보유가
되므로 종목별 변화는 **기초일 종목 기준으로 AP 를 찾아** 계산한다.

### 구현 위치

- `modules/data_loader.py` — `_ACWI_SPLIT_DATASETS` / `load_acwi_equity_split(as_of)`
- `api/schemas/brinson.py` — `BrinsonBmEquitySplitDTO` / `...RowDTO` + 응답 `bm_equity_split`
- `api/services/brinson_service.py` — `_build_bm_equity_split` / `_order_split_rows` / `_TAA_SO_TO_BUCKET`
- `web/src/tabs/BrinsonTab.tsx` — `eqSplit0` 로 `bmComps0['해외주식']` 교체 + 하단 주석, 표0/표1 좌우 교대

---

## 성과분석 엑셀 — Overview 조회구간 차트 (2026-08-24)

Overview 시트 차트가 **설정후 고정** 하나뿐이라, 우측에 **조회구간 차트**를 추가했다.
조회기간이 설정후와 같으면 **1개만** 그린다.

- 시계열 시트에 **조회 기초일=0% 로 리베이스한 별도 컬럼 블록**을 만들어 차트가 참조한다.
  기존 수익률 컬럼은 설정일 기준이라 그대로 쓰면 누적 레벨(+78%)이 찍힌다.
- ★ **설정후 판정은 설정일 기준.** nav 시계열 첫 행이 **설정일 T-1 합성행**(base 1000,
  [[reference_inception_base_1000]])이라 `행 index > 0` 으로 보면 설정후 조회에서도
  index=1 이 되어 **같은 차트가 2개** 나온다.
- 1년 이하 구간은 x축이 뭉개져 `yy-mm-dd` 로 표기.
- 검증: YTD 2개 / 3M 2개 / **설정후 1개** / 2JM23(벤치 없음) 펀드 단독 2개.
  조회구간 블록 값 = 2025-12-31 0% 시작, 기말 AP +27.84 / BM +17.88 / 초과 +9.95%p.
- legend `초과수익률` → **`초과수익률(우)`** (보조축 표기, 2026-08-24 사용자 지정).
- `api/services/brinson_export_service.py` — `_ov_chart()` 헬퍼로 차트 생성 일원화.

---

## 거래내역 — 변화율 토글 / 줌 리셋 (2026-08-24)

`변화율` 토글은 **드래그로 두 시점 지수 변화율을 재는 자**다. 버튼만 눌러선 화면이
안 바뀌고 **드래그해야** `▲ +8.24%` 라벨 + 점선이 그려진다(ON = `dragmode:'select'`,
OFF = 드래그 줌 + y auto-fit).

### ★ Plotly select 모드에서 더블클릭 리셋이 안 먹는다

실측으로 확인한 제약 둘:

1. select 모드에서 Plotly 는 더블클릭을 **선택 해제로 소비**하고
   `plotly_relayout`·`plotly_doubleclick` 을 **하나도 안 보낸다**(이벤트 0건).
2. 드래그 레이어가 mousedown 에 `preventDefault` 를 걸어 **native `dblclick` 자체가
   발생하지 않는다** — `document` capture 단계에서도 안 잡힌다. `onDoubleClick` 무용.

게다가 축에 `range`+`autorange:false` 를 명시하므로 Plotly 의 'reset' 기준값이 매
렌더마다 현재 범위로 갱신돼 no-op 이 된다. → **살아있는 mousedown 2회의 간격(400ms)·
좌표(6px)로 직접 판정**해 `xr`/`yRange`/`measure` state 를 비운다(단일 소스).
더블클릭 부산물인 **폭 0 선택**은 `p0.date === p1.date` 로 걸러 `0.00%` 라벨을 막는다.

증상(2026-08-24 사용자 리포트): OFF 에서 줌 → ON 전환 시 줌이 고정되고, OFF 로 풀어도
다시 ON 하면 줌이 되살아남 — 셋 다 같은 뿌리(묵은 `xr` 재적용).
**ON 전환 시 줌은 유지**(확대 구간 내 측정), 더블클릭이면 리셋 — 이게 확정 동작이다.

`web/src/components/charts/SecurityReturnChart.tsx`

---

## 워밍업 완료 배너가 안 사라지던 문제 (2026-08-25)

`WarmupGate` 가 `done_with_errors` 상태를 **조건 없이** 그려서 빨간 띠가 하루 종일
붙어 있었다. 서버 `status` 는 **재기동 전까지 그대로 남는다**(폴링은 멈춰도 캐시된
응답으로 계속 렌더). → `finished_at` 기준 **5분(TTL) 이내만** 표시 + **× 닫기** 추가.

- 실패 스텝은 해당 탭 진입 시 자동 로드되므로 시간이 지나면 알릴 내용이 없다.
- `finished_at` 없으면(구버전 응답) 표시 — fail-open.
- `web/src/components/common/WarmupGate.tsx`

### 미해결: 매일 첫 기동 시 `06X08 · 편입종목` 90초 타임아웃 1건

- 편입종목 빌드 끝에 `duration_fetcher` 가 **외부 HTTP(KIM 사이트)로 듀레이션을
  조회**한다(종목당 timeout 15s, 날짜별 파일 캐시 `market_research/data/duration_archive/`).
- **06X08 이 `FUND_LIST` 첫 펀드**라 그날의 콜드 조회 11건을 첫 스텝에서 다 떠안는다.
  실측 2026-08-25: 캐시 파일 생성 08:01~08:05(약 4분) vs `_ESSENTIAL_STEP_TIMEOUT=90`.
- 타임아웃 나도 **작업 스레드는 계속 돌아 캐시를 채우므로** 이후 펀드는 정상(78/78).
  캐시가 찬 뒤 재측정 = 06X08 9.0s / 08K88 6.9s / 07G04 23.8s. **기능 영향 없음.**
- 없애려면 **듀레이션 캐시 채우기를 별도 첫 스텝으로 분리**(`fetch_all()` 1회, 타임아웃
  넉넉히)가 원인을 정확히 겨냥한다. 타임아웃 상향(90→240)은 감지력이 둔해진다.

---

## ★ 시장코멘트 = 총론 + 자산군별 · 수치는 원칙 배제 (2026-09-02 사용자 지시)

`_market` 본문 구조를 **테마 기준 3~4문단 → 총론 1문단 + 자산군별 문단**으로 바꾸고,
09_Research_Synthesis 를 **전망(Step 3)·시드에도 직접** 공급한다.

### 왜 — 축이 두 번 깨지고 있었다

```
09 (자산군별)  →  [작성규칙 2 "자산별 개별 나열 금지"]  →  산문으로 평탄화
              →  market_seed 가 그 산문에서 자산군을 **재구성**
```

시드 프롬프트가 `"원문에 있는 사실만 쓰고 새 사실을 넣지 마세요"` 라, **본문 압축에서
탈락한 09 내용은 구조적으로 복구 불가**였다. 이제 09 → 시장코멘트 → 시드가 같은
자산군 축(`core/asset_class.MARKET_ORDER`)을 공유한다.

- 단절 3곳 중 2곳을 메웠다: **Step 3 `outlook_prompt`** 와 **`build_seed`** 에 09 주입.
  (펀드 코멘트는 여전히 09 를 직접 보지 않는다 — 시드를 통해서만 받는다. 의도된 설계.)
- ⚠ `structure_instruction` 은 client 운용보고(매크로) 탭에 그대로 노출되는 텍스트다.
  **발송본 톤이 바뀌는 변경**이라 사용자 승인 후 적용했다.

### 수치 정책 — "허용 목록"이 아니라 **공급**

| | |
|---|---|
| 원칙 | 숫자를 쓰지 않는다. 방향과 맥락으로 서술 |
| 예외 | `core/market_levels.py` 레벨 블록의 **레벨·기간중 고저**, 정책금리 등 제도 수치 |
| 금지 | evidence 카드에서 온 **특정 시점 수치** — 일간 등락률, 개별 기업 실적치·수주잔고, 특정일 발표치 |

★ **금지만 걸면 환각이 그 자리를 채운다.** 실측 2건이 같은 교훈이다 —
시드 규칙 4가 "퍼센트 대신 레벨로" 를 요구하면서 레벨을 안 줘서 모델이 레벨을
지어냈다(달러/원 "1,530원대", 실제 1,368.60). 그래서 규칙과 **레벨 블록 공급**은
반드시 같이 간다.

- ref 규칙에서 `"수치가 포함된 사실 서술에는 가급적 ref 명시"` 를 **삭제**했다 —
  기사 수만큼 수치를 끌어오는 압력이었다.
- `## 좋은 코멘트 예시` 도 같이 고쳤다. **예시가 규칙을 어기면 예시가 이긴다** —
  종전 예시에 "KOSPI는 +14.4%, KOSPI200은 +16.3%" 가 박혀 있었다.

### 진단 실측 (2026-08 운영 승인본)

| 단계 | 분량 | 수치 | 밀도 |
|---|---|---|---|
| 09 주입분(§1+§3+§4, 8자산군) | 7,198자 | 31개 | 232자당 1개 |
| **시장코멘트** | 2,517자 | **23개** | **109자당 1개** |
| 시드(시장동향) | 860자 | 4개 | 215자당 1개 |

- 23개 중 **14개가 09 에 없는 것** = evidence 카드(뉴스) 출처.
- ★ `asset_movement_anchors` 의 월간 변화 수치는 **4/4 전부 미인용**이었다.
  정답 숫자가 프롬프트에 있는데도 안 쓰고 뉴스 시점 수치를 썼다.

### ⚠ anchors 국내주식(`MSCI_KOREA`)은 KOSPI 가 아니다

`SCIP dataset 144` = **iShares MSCI South Korea ETF (EWY-US)** — 미국 상장 ETF다.
라벨만 "MSCI Korea" 고 `macro_data.py` 의 `desc` 가 괴리를 가린다
([[reference_macro_etf_proxy]] 계열 함정).

```
2026-08 월간   MSCI_KOREA +15.12%  vs  KOSPI +3.40%      (4.4배)
2026-07-31     MSCI_KOREA  -2.62%  vs  KOSPI +17.91%     (부호 역전 — 미국장이라 하루 리드)
```

USD 표시(환효과)+거래시간 시차로도 7%p 가 안 풀린다. **국내주식 anchors 를 월간
서술 근거로 쓰지 말 것.** 사용처는 `asset_movement_anchor.py` 와 `engine.py`
국면감지뿐이고 Brinson·펀드 코멘트·대시보드와는 무관하다.
(engine 국면감지가 "국내주식" 이라 믿고 이 시리즈를 보는 건 **미해결 항목**.)

### 시장 레벨 사실 — `core/market_levels.py` 단일 소스

시드에만 있던 `_market_level_facts` 를 core 로 올려 **debate synthesis 와 공유**한다.
한쪽만 고치면 같은 달 보고서 안에서 레벨이 갈린다. `market_seed` 는 re-export 별칭만
남긴다(회귀가드 5건이 그 경로를 검사).

- 시리즈 = **자산군당 대표 1개** (`LEVEL_SERIES`): KOSPI 253/15 · S&P500 271/15 ·
  미국채10년 2/7 · 금 408/48 · 달러/원 31/6 · DXY 105/48.
  숫자를 늘리려는 게 아니라 자산군별 서술이 인용할 정답을 주는 것이라 그 이상 늘리지 않는다.
- ⚠ **국내채권은 비워 둔다** — SCIP 에 국고채 yield 시리즈가 없고 KIS/KAP 은 전부 지수라
  "국고채 10년 x.xx%" 인용의 근거가 못 된다. 규칙이 "블록에 없으면 레벨을 쓰지 말라"로 받는다.
- ⚠ **금리는 bp, 나머지는 %.** `kind='yield'` 분기 — 4.71%→4.75% 는 **+4bp** 이지
  "+0.72%" 가 아니다. 퍼센트로 쓰면 오해를 부른다.
- ⚠ `period_bounds`(기초 = 직전 기간의 마지막 날)를 `core/period_window.py`(08K88
  예외 창)와 **합치지 말 것** — 다른 규약이다 ([[reference_pa_period_start_offbyone]] 계열).

### ★ 고저는 **종가 기준**이다 — 장중이 아니다

사내 DB 어디에도 지수 장중 고저가 없다. 전수 확인:

```
SCIP 253(KOSPI) dataseries = 9(TR)·15(FG Price)·48(PX_LAST)  → 전부 종가
back_dataseries 이름 HIGH|LOW|INTRA                          → 0건
dt.MI_DJISU(eod00d_*) · BMJISU · MI_DSJISU · BB_IDXEVT       → 고저 컬럼 없음
solution.sol_index_inform                                    → 매핑 테이블(OHLC 없음)
```

- 2026-09-02 사용자 결정: 블룸버그 `PX_HIGH`/`PX_LOW` 수집안은 **채택하지 않는다**.
- → 블록 라벨에 "종가 기준" 을 박고 프롬프트에 **장중 표현 금지**를 건다.
  ⚠ 발송본 관행이 "KOSPI가 장중 5,200포인트 선까지 밀리는"(승인 원고)이라
  가만두면 종가 숫자가 장중 표현에 붙는다.
- ★ **서술의 기준(장중/종가)을 확인하기 전에 시계열로 반증하지 말 것.**
  2026-08 시드의 "KOSPI 7,200 터치" 를 월중 최고 **종가** 6,977.94 로 환각 판정했다가
  철회했다 — 장중 얘기였고 종가 데이터로는 **검증 불가**가 정확한 상태다.

### 09 발췌 — 경계 컷 + 자산군 상한

- `_cut_at_sentence` — 산문은 **문장 경계**, claim 은 **줄 경계**에서 끊는다.
  종전 `[:900]`/`[:700]` 하드컷은 2026-08 기준 8개 자산군 전부에서 마지막 리스크
  문장을 반쪽으로 잘랐다.
- `research_synthesis_max_assets` **8 → 12**. 2026-08 의 09 파일이 정확히 8개라
  상한과 딱 맞물려 있었다 — 자산군이 하나만 늘어도 `sorted()` 마지막인
  **환율FX 가 조용히 통째로 누락**된다.

### 회귀가드

`market_research/tests/test_asset_structured_comment.py` (22건) — 문단 구조·수치 정책·
09 주입 3곳·경계 컷·자산군 상한·레벨 단일 소스·금리 bp 단위를 고정.
