@echo off
REM =====================================================================
REM  OCIO watchdog launcher that CAPTURES THE EXIT CODE and RELAUNCHES.
REM
...
REM
REM  RESTART LOOP (2026-08-07): the wrapper used to exit together with the
REM  watchdog, so a midnight kill left the server unguarded until a human
REM  noticed - on 08-07 that was an 8 hour gap. Now the wrapper relaunches it.
REM  Two guards keep the loop from fighting deliberate shutdowns:
REM    - stop_ocio_server.bat drops .cache\watchdog_stop.flag BEFORE it kills
REM      the watchdog, so "stop everything" stays stopped.
REM    - rc=0 means the script left its own loop (normally the duplicate
REM      instance mutex). Relaunching that would spin two wrappers forever.
REM =====================================================================
setlocal
set "ROOT=%~dp0.."
for %%i in ("%ROOT%") do set "ROOT=%%~fi"
set "STOP=%ROOT%\.cache\watchdog_stop.flag"

REM A stale flag (wrapper killed alongside the watchdog) must not block startup.
...
if exist "%STOP%" (
    del /f /q "%STOP%"
    >>"%ROOT%\logs\watchdog.log" echo %TS% ^| stop requested - wrapper exiting, no relaunch
    goto :done
)
if "%RC%"=="0" (
    >>"%ROOT%\logs\watchdog.log" echo %TS% ^| rc=0 already-running/graceful - wrapper exiting, no relaunch
    goto :done
)

>>"%ROOT%\logs\watchdog.log" echo %TS% ^| relaunching watchdog in 10s
REM ping, not timeout: timeout aborts when stdin is not a real console and the
REM wrapper runs hidden from wscript.
