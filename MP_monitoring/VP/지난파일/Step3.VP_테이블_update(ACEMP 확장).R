...
VP_fund_name <- bind_rows(VP_fund_name,VP_fund_name_ACETDF,VP_fund_name_기타신상품) %>% 
...
MP_rebalancing_historical <- sol_VP_rebalancing_inform %>%
  ...
  select(리밸런싱날짜,펀드설명,port,ISIN,weight,rebalancing_reason,for_ACE,Recalculate_date)
...
MP_rebalancing_historical %>% 
  ...
  select(리밸런싱날짜,펀드설명,port, for_ACE, Recalculate_date,dataset_id ,dataseries_id, region,
         weight,port,hedge_ratio,  rebalancing_reason, cost_adjust ) ->MP_monitor_universe
...
if(length(new_fund)!=0){
  ...
  sol_MP_released_inform %>% 
    filter(펀드설명==new_fund) ->new_fund_information
