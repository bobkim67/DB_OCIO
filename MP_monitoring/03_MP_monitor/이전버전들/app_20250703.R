...
source("./03_MP_monitor/데이터 loading 모듈 20250703.R")
source("./03_MP_monitor/데이터 Preprocessing 모듈 20250703.R")
source("./03_MP_monitor/inform_module_20250703.R")
source("./03_MP_monitor/performance_module_20250703.R")
source("./03_MP_monitor/position_module_20250703.R")
    ...
    header = tagList(
      conditionalPanel(
        ...
        div(
          ...
          div(
            checkboxGroupInput("TDF_BF", label = "유형 선택:", inline = TRUE, choices = unique(Fund_Information$구분), selected = c("TDF", "BF", "ACETDF"))
          )
        )
      )
    ),
