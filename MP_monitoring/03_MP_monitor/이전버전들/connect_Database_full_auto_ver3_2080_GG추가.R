  ...
  data %>%
    ...
    dplyr::mutate(filtered_first_date = 
                    case_when(
                      ...
                      from_when == "최근 1개월" ~ input_date - months(1),
                      from_when == "최근 3개월" ~ input_date - months(3),
                      from_when == "최근 6개월" ~ input_date - months(6),
                      from_when == "Base date" ~ base_date-days(1),
                      TRUE ~ NA_Date_
                    )
    ) %>%
    mutate(filtered_first_date = if_else(last_day_of_month ==TRUE, 
...
MP_historical<- tbl(con_SCIP,"back_datapoint") %>% 
  filter(dataset_id %in% mp_dataid) %>%   
