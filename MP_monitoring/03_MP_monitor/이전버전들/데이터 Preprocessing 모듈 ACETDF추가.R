...
AP_performance_preprocessing_ACETDF <- VP_performance_preprocessing_ACETDF %>% mutate(수정기준가=1000)
...
AP_ACETDF_asset_adjust <- VP_ACETDF_asset_adjust %>% mutate(시가평가액=1000) %>% mutate(순자산=1000)
...
AP_VP_MP_diff_summarised %>%
  ...
  ungroup() %>% 
  rowwise() %>% 
  # ACETDF 추가 수정: daily_weight_AP/normalize_AP 대신 if_else(normalize_AP==0, 0, daily_weight_AP/normalize_AP)
  # mutate(min_AP_VP = min(if_else(normalize_AP==0, 0, daily_weight_AP/normalize_AP),daily_weight_VP/normalize_VP),
  ...
  reframe(`괴리율(VP&MP, 주식+대체 대분류)` = (normalize_VP-normalize_MP) ,
          ...
          # `괴리율N.(AP&MP,주식+대체 소분류)`= max(( if_else(normalize_AP==0,0,daily_weight_AP/normalize_AP) - daily_weight_MP/normalize_MP )) ,#`max[Normalize_(AP-MP)주식+대체_소분류]`
