...
MP_LTCMA_all_comb %>% 
  ...
  filter(기준일자>=설정일) %>% 
  ungroup() %>% 
  select(-기준값) -> MP_performance_preprocessing_final
...
#---- wrap의 BM도 MS Growth와 동일하게 주식:채권 7:3으로 수동 설정 ------------------------------------------------------------------------
  mutate(weight = if_else((펀드설명 =="SK EMP70" & 자산군=="주식"),0.7,weight )) %>% 
  mutate(weight = if_else((펀드설명 =="SK EMP70" & 자산군=="채권"),0.3,weight )) %>% 
#------------------------------------------------------------------------------------------------------------------------------------------
...
BM_weight_all_comb %>% filter(weight>0) %>%
  ...
  mutate(수정기준가 = 수정기준가/기준값*1000 ) %>% 
  filter(기준일자>=설정일) %>% 
  ungroup()  -> BM_performance_preprocessing_final
...
crossing(기준일자 =unique(AP_asset_adjust$기준일자),
         펀드설명 =  Fund_Information %>% filter(구분 %in% c("TDF","BF","ACE TDF", "Wrap")) %>% pull(펀드설명),
         universe_criteria %>%
           filter(자산군_대!="유동성") %>% 
...
duration_historical_prep<- 
  ...
  mutate(duration = if_else(duration ==0, NA, duration)) %>% #가끔 업데이트가 0으로될떄있느데 차라리 전일자껄로 사용하게끔
