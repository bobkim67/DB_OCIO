...
source("./03_MP_monitor/데이터 loading 모듈 20250620.R")
source("./03_MP_monitor/데이터 Preprocessing 모듈 ACETDF(ETF).R")
source("./03_MP_monitor/inform_module.R")
source("./03_MP_monitor/performance_module_ACETDF_통합.R")
# source("./03_MP_monitor/position_module_ACETDF_v2.R")
source("./03_MP_monitor/position_module_ACETDF_통합.R")
source("./05_Performance_Attribution/brinson_shiny_default_v6.R")
    ...
    header = tagList(
      conditionalPanel(
        ...
        div(
          ...
          div(
            checkboxGroupInput("TDF_BF", label = "유형 선택:", inline = TRUE, choices = unique(Fund_Information$구분), selected = c("TDF", "BF","ACE TDF"))
          )
        )
      )
    ),
