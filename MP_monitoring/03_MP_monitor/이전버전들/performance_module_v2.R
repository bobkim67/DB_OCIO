...
performanceServer <- function(id, global_date) {
  moduleServer(id, function(input, output, session) {
    ...
    #modalContent <- reactiveVal()
    
    # observeEvent 안에서 rmarkdown::render를 호출하고
    # 결과를 modalContent reactiveVal에 저장합니다.
    # observeEvent(input$showModal, {
    #   content <- rmarkdown::render("03_MP_monitor/test.Rmd")
    #   modalContent(content)  # modalContent reactiveVal을 업데이트합니다.
    #   showModal(modalDialog(
    #     title = "각 지표별 계산 방법",
    #     uiOutput(ns("modalContent")),
    #     size = "l",
    #     easyClose = TRUE,
    #     footer = NULL
    #   ))
    ...
    # modalContent reactiveVal의 현재 값을 HTML로 렌더링하여 반환합니다.
