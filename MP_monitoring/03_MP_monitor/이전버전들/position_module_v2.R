  ...
  moduleServer(id, function(input, output, session) {
    ...
    replication_rate <- reactive({
      ...
      AP_VP_MP_diff %>%
        ...
        mutate(자산군_대 = if_else(자산군_대 %in% c("대체","주식"),"주식 및 대체",자산군_대)) %>%
        group_by(기준일자,펀드설명,자산군_대) %>%
        mutate(daily_자산군_대_AP = sum(daily_weight_AP),
               daily_자산군_대_VP = sum(daily_weight_VP),
               daily_자산군_대_MP = sum(daily_weight_MP)) %>%
        group_by(기준일자,펀드설명,자산군_소) %>%
        reframe(daily_weight_AP = sum(daily_weight_AP) ,
                daily_weight_VP = sum(daily_weight_VP) ,
                daily_weight_MP = sum(daily_weight_MP) ,
                daily_자산군_대_AP = daily_자산군_대_AP[1],
                daily_자산군_대_VP = daily_자산군_대_VP[1],
                daily_자산군_대_MP = daily_자산군_대_MP[1],
                자산군_대= 자산군_대[1]
        ) %>%
        group_by(기준일자,펀드설명,자산군_소) %>%
        reframe(normalized_portion_AP = daily_weight_AP/daily_자산군_대_AP,
                normalized_portion_VP = daily_weight_VP/daily_자산군_대_VP,
                normalized_portion_MP = daily_weight_MP/daily_자산군_대_MP,
                자산군_대 = 자산군_대[1]) %>%
        mutate(across(.cols = contains("normalized"),.fns = ~tidyr::replace_na(.x,0))) %>%
        ungroup() %>%
        rowwise() %>%
        mutate(AP_VP_min = min(normalized_portion_AP,normalized_portion_VP),
               AP_MP_min = min(normalized_portion_AP,normalized_portion_MP)) %>%
        group_by(기준일자,펀드설명,자산군_대) %>%
        reframe(`AP & VP` = sum(AP_VP_min),
                `AP & MP` = sum(AP_MP_min))->replication_rate
    ...
    output$replication_stock_AI <- renderPlotly({
      ...
      long_data <- replication_rate() %>%
        filter(자산군_대 == "주식 및 대체") %>%
        pivot_longer(cols = starts_with("AP"), names_to = "구분", values_to = "복제율") %>%
        mutate(
          impact_color = case_when(
            복제율 < 0.95 ~ "red",    # 복제율이 0.95 미만일 때 빨간색
            복제율 < 0.97 ~ "#FFA500", # 복제율이 0.97 미만일 때 주황색
      ...
      p <- ggplot(long_data, aes(x = 펀드설명, y = 구분)) +
        ...
        labs(title = "주식 및 대체 복제율", x = "펀드설명", y = "") +
        theme_minimal() +
