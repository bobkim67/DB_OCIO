...
universe_criteria <- 
  ...
  bind_rows(
    tribble( ~종목코드, ~종목명,~자산군_대,~자산군_중,~자산군_소,
             ...
             "KR7385540000", "RISE 종합채권(A-이상)액티브",  "채권", "한국 채권", "한국 종합채권"
...
return_performance_shallow <- function(data, input_date, from_when, base_date = NULL) {
  ...
  data %>%
    ...
    mutate(설정직전날= if_else(sum(설정일>filtered_first_date)!=0,TRUE,FALSE )) %>% 
...
url_mapping = list(
  "MXWD Index"    = list("dataset_id"= as.integer(35), "dataseries_id"= as.integer(9)),
  "KOSPI2 Index"  = list("dataset_id"= as.integer(145), "dataseries_id"= as.integer(9)),
  "KBPMMMIN Index"= list("dataset_id"= as.integer(146), "dataseries_id"= as.integer(9)),
  "M1EF Index"    = list("dataset_id"= as.integer(147), "dataseries_id"= as.integer(9)),
...
duration_historical_prep<- 
  ...
  #mutate(across(.cols =!(contains("한국")) , .fns = ~zoo::na.locf(.x) )) %>% 
...
AP_VP_MP_diff_summarised %>% 
  ...
  mutate(`펀드듀레이션(AP/VP)`=AP_duration/VP_duration,
         ...
         `채권듀레이션(AP/MP)`=AP_duration.N/MP_duration.N)  ->bond_duration_replicate
