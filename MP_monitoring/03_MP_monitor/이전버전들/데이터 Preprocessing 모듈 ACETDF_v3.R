...
AP_VP_MP_diff_summarised %>%
  ...
  mutate(위험자산비중_상한 = case_when(펀드설명 %in% c("MS GROWTH","TDF2080") ~ 1 , 
                               펀드설명 %in% c("TIF") ~ 0.4 ,
                               str_detect(펀드설명,("ACE")) ~ 1 ,
                               TRUE ~ 0.8 
                               )) %>% 
  filter(new_대분류!="채권") %>% 
  group_by(기준일자,펀드설명) %>% 
...
replicate_long_form<- replicate_disparate_rate %>%
  ...
  mutate(구분= factor(구분,levels=c("괴리율(VP&MP, 주식+대체 대분류)","괴리율(AP&MP, 주식+대체 대분류)",
                                ...
                                "위험자산비중 상한","위험자산비중(VP)","위험자산비중(AP)" ))) %>% 
  mutate(구분= fct_rev(구분))  %>% 
  mutate(복제율 = str_detect(구분,"복제율"),
         위험자산 = str_detect(구분,"위험자산")
  ) %>%
  group_by(기준일자, 펀드설명) %>%
  mutate(위험자산_signal_orange_AP = `복제율&괴리율`[구분=="위험자산비중 상한"]*0.98- `복제율&괴리율`[구분=="위험자산비중(AP)"] < 0 ,
         위험자산_signal_red_AP = `복제율&괴리율`[구분=="위험자산비중 상한"]*1- `복제율&괴리율`[구분=="위험자산비중(AP)"] < 0 ,
         위험자산_signal_orange_VP = `복제율&괴리율`[구분=="위험자산비중 상한"]*0.98- `복제율&괴리율`[구분=="위험자산비중(VP)"] < 0 ,
  ...
  mutate(
    ...
    impact_color = case_when(
      (위험자산==TRUE & 위험자산_signal_red_AP) ~ "red",
      (위험자산==TRUE & 위험자산_signal_orange_AP) ~ "#FFA500",
      (위험자산==TRUE & 위험자산_signal_red_VP) ~ "red",
      (위험자산==TRUE & 위험자산_signal_orange_VP) ~ "#FFA500",
      (위험자산==TRUE ) ~ "white",
