...
calculate_BM_results_bulk <- function(dataset_id_vector, dataseries_id_vector, region_vector,
  ...
  #   left_join(Data_List_v2 %>% select(dataset_id=id,name=colname_backtest), by = join_by(dataset_id_vector==dataset_id)) %>% 
  #   mutate(name = str_glue("{name}{if_else(region_vector != 'KR', '(t-1)', '')}{if_else(hedge_ratio_vector != 0 & region_vector != 'KR', '*(USDKRW)', '')}"))
  for_pulling_universe_data <- Data_variable_list %>% 
