...
#(MP) 여기에 K-ladder 기준가 불러와서 붙이기.. 등등 ------------------------------------------
# k-ladder펀드 기준가를 영업일만 필터링한 다음, T-1처리해야됨. 펀드회계상 그렇게됨..
...
USDKRW %>% 
  left_join(F_USDKRW_Index) %>% 
  mutate(across(-기준일자,.fns = ~.x/lag(.x)-1)) %>% 
  mutate(환헷지_및_비용처리 = (1+`USD/KRW`-`F_USD/KRW`)/(1 +`USD/KRW`)-1 ) %>%    #환헷지 비용 나누기 환율 이미 곱한것
  select(기준일자,환헷지_및_비용처리) %>% 
  filter(!is.na(환헷지_및_비용처리))-> hedge_cost_adjust


MP_LTCMA_all_comb %>% 
  left_join(MP_performance_preprocessing , by = join_by(기준일자,ISIN,자산군_대,자산군_소)) %>% 
  left_join(hedge_cost_adjust) %>% 
  mutate(전일대비등락률 = if_else(hedge_ratio ==1, (1+전일대비등락률)*(1+환헷지_및_비용처리)-1,전일대비등락률 )) %>% 
  group_by(기준일자,펀드설명) %>% 
  reframe(전일대비등락률 = sum(weight*전일대비등락률),
          last_rebalance = 리밸런싱날짜[1]) %>% 
  left_join(Flagship_FUNDS_DES %>% select(펀드설명, 모니터링시작일)) %>% 
  group_by(펀드설명) %>%
  filter(기준일자>=모니터링시작일) %>% 
...
#MP_performance_preprocessing_final %>% view()
# _BM ----------------------------
...
BM_weight<- 
  ...
  mutate(weight = if_else((펀드설명 =="SK EMP70"  & 자산군=="주식"),0.7,weight )) %>% 
  mutate(weight = if_else((펀드설명 =="SK EMP70"  & 자산군=="채권"),0.3,weight )) %>% 
  ...
  mutate(자산군 = case_when(
    ...
    자산군 == "주식" ~ "M2WD Index",
    자산군 == "채권" & 펀드설명 %in% c("MS GROWTH", "MS STABLE","Golden Growth","SK EMP70") ~ "KST0000T Index",#"KIS 종합 총수익지수",
...
Market_Cap_GV %>% 
  ...
  group_by(기준일자) %>% 
  mutate(비중 = `marketcap(T-1)`/sum(`marketcap(T-1)`)) %>% ungroup() %>% 
  pivot_wider(id_cols = 기준일자,names_from = name,values_from = 비중) %>% 
  timetk::pad_by_time(.date_var = 기준일자,.fill_na_direction = "down",.by = "day")  -> historical_MarketCap_GV
...
union(
  ...
  filter(!str_detect(종목,"FX")) %>% 
  filter(!str_detect(종목,"LU")) %>% 
  filter(!str_detect(종목명,"증거금")) 
...
AP_asset_adjust<- bind_rows(
  ...
  AP_asset_adjust %>% 
    filter(!str_detect(자산군_소,"분해필요")| str_detect(종목,"0322800") ) ,
...
pulling_모자구조 %>% 
  filter(!is.na(운용모펀드)) %>%
  filter(FUND_CD %in% (Flagship_FUNDS_DES %>% 
                         filter(모자구조 ==TRUE) %>% pull(펀드_AP))) %>% 
  select(FUND_CD,모펀드통합) %>% unlist() %>% unname() %>% unique()->monson_fund_list

AP_asset_adjust %>% 
  filter(펀드 %in% monson_fund_list) -> 모자_related_all

bind_rows(
  
  모자_related_all %>% 
    mutate(종목 = str_remove_all(종목, "0322800")) %>% 
    filter(nchar(종목)==12) %>% 
    filter(!(펀드 %in% unique(pulling_모자구조$운용모펀드))) ,
  
  모자_related_all %>% 
    mutate(종목 = str_remove_all(종목, "0322800")) %>% 
    filter(nchar(종목)!=12) %>% 
    select(-contains("자산군"),-종목명) %>% 
    inner_join(모자_related_all %>% 
                 filter(펀드 %in% unique(pulling_모자구조$운용모펀드)) %>% 
                 rename(모_시가평가액 = 시가평가액, 모_순자산 = 순자산), 
               by = join_by(기준일자, 종목 ==펀드),
               relationship = "many-to-many") %>% 
    select(-종목) %>% 
    rename(종목=종목.y) %>% 
    mutate(시가평가액_조정결과 = 모_시가평가액 *시가평가액/모_순자산 ) %>% 
    select(-시가평가액) %>% 
    rename(시가평가액 = 시가평가액_조정결과) %>% 
    select(all_of(colnames(모자_related_all))) 
  
) %>% 
  group_by(기준일자,펀드,종목) %>% 
  reframe(종목명 = 종목명[1],
          시가평가액 = sum(시가평가액,na.rm = TRUE),
          순자산 =순자산[1],
          자산군_대 = 자산군_대[1],
          자산군_중 = 자산군_중[1],
          자산군_소 = 자산군_소[1]) ->모자_look_through

AP_asset_adjust<- AP_asset_adjust %>% 
  filter(!(펀드 %in% monson_fund_list)) %>% 
  bind_rows(모자_look_through)
...
crossing(기준일자 =unique(AP_asset_adjust$기준일자),
  ...
  filter( !(is.na(daily_weight_AP ) & is.na(daily_weight_VP) & is.na(`daily_weight_VP*`) & is.na(daily_weight_MP))) %>%
  ...
  mutate(across(starts_with(c("daily_weight")), ~replace_na(., 0))) %>% 
  filter((daily_weight_AP+daily_weight_VP+daily_weight_MP)!=0 ) %>% 
...
AP_VP_MP_diff_summarised %>%
  ...
  mutate(
    ...
    `채권비중_VP*` = sum(`daily_weight_VP*`[new_대분류=="채권"]),
    채권_위험자산비중= sum(daily_weight_AP[자산군_소%in%c("미국 하이일드채권")])
  ) %>%  
  mutate(위험자산비중_상한 = case_when(펀드설명 %in% c("MS GROWTH","TDF2080","ACETDF2080","SK EMP70") ~ 1 , 
                               펀드설명 %in% c("TIF") ~ 0.4 ,
                               TRUE ~ 0.8 
  )) %>% 
  filter(new_대분류!="채권") %>% 
  ...
  ungroup() %>% 
  # ACETDF 추가 수정: (pmin 사용하여 벡터화)
  mutate(min_AP_VP = pmin(daily_weight_AP/normalize_AP,daily_weight_VP/normalize_VP),
         `min_AP_VP*` = pmin(daily_weight_AP/normalize_AP,`daily_weight_VP*`/`normalize_VP*`),
         `min_주식+대체_대분류`= pmin(normalize_AP,normalize_VP),
         `min_주식+대체_대분류*`= pmin(normalize_AP,`normalize_VP*`),
         min_채권_대분류= pmin(채권비중_AP,채권비중_VP),
         `min_채권_대분류*`= pmin(채권비중_AP,`채권비중_VP*`),
  ) %>% 
  ...
  reframe(`괴리율(VP&MP, 주식+대체 대분류)` = normalize_VP-normalize_MP,
          ...
          `위험자산비중(AP)` = `주식+대체비중_AP`[1]+채권_위험자산비중[1],
          `위험자산비중(VP)` = `주식+대체비중_VP`[1]+채권_위험자산비중[1],
          `위험자산비중(VP*)` = `주식+대체비중_VP*`[1]+채권_위험자산비중[1]
...
replicate_long_form<-
  ...
  filter( ! (str_detect(구분,"\\*") & str_detect(구분,"복제율"))  ) %>%
  ...
  mutate(
    ...
    impact_color = case_when(
      ...
      (str_detect(구분,"VP")&위험자산==TRUE & 위험자산_signal_red_VP) ~ "red",
      (str_detect(구분,"VP")&위험자산==TRUE & 위험자산_signal_orange_VP) ~ "#FFA500",
      #(위험자산==TRUE ) ~ "white",
      ((펀드설명=="TIF"&위험자산==FALSE) &(복제율==FALSE) &(abs(`복제율&괴리율`) >= 0.03)) ~ "red",
      ((펀드설명=="TIF"&위험자산==FALSE) &(복제율==FALSE) &(abs(`복제율&괴리율`) >= 0.02))  ~ "#FFA500",
      ((복제율==TRUE&위험자산==FALSE) &(abs(`복제율&괴리율`) <= 0.95)) ~ "red",
      ((복제율==TRUE&위험자산==FALSE) &(abs(`복제율&괴리율`) <= 0.97))  ~ "#FFA500",
      ((복제율==FALSE&위험자산==FALSE) &(abs(`복제율&괴리율`) >= 0.05)) ~ "red",
      ((복제율==FALSE&위험자산==FALSE) &(abs(`복제율&괴리율`) >= 0.03))  ~ "#FFA500",
...
duration_historical_prep<- 
  ...
  bind_rows(crossing(기준일자 = 최근영업일,
                     자산군_소 = bond_url_mapping$name) %>% 
              mutate(duration = NA )) %>% 
  group_by(기준일자,자산군_소) %>% 
  filter(row_number()==1) %>% 
  group_by(자산군_소) %>% 
  fill(-기준일자, .direction = "downup") %>%  
  ungroup()
