...
# 모든 가격정보 (tot return)
ALL_historical<- tbl(con_SCIP,"back_datapoint") %>% 
  filter(dataseries_id==6) %>%   
  filter(timestamp_observation>="2022-10-03") %>% 
  collect() 

ALL_historical_historical_prep<- ALL_historical %>% 
  left_join(Data_List %>% select(dataset_id=id,name = ISIN) %>% distinct(),by = join_by(dataset_id)) %>% 
...
position_VP<- position_VP_summarised %>% 
  group_by(기준일자,펀드,자산군_대) %>% 
  reframe(daily_weight = sum(daily_weight))

position_MP<- position_MP_summarised %>% 
  group_by(기준일자,펀드,자산군_대) %>% 
  reframe(daily_weight_MP = sum(daily_weight))
...
AP_VP_MP_diff_PA_weight <-
  ...
  left_join(
    left_join(position_MP %>% rename(펀드설명=펀드),
                ...
                mutate(자산군_대 = if_else(str_detect(symbol,"M2WD") ,"주식","채권")) %>% 
...
return_information_PA <- 
  ...
  bind_rows(
    ...
    MP_VP_BM_results_core %>%
      filter(port == "MP") %>%
      select(펀드설명,기준일자,`Weight_drift(T-1)`) %>%
      unnest_wider(`Weight_drift(T-1)`) %>%
      pivot_longer(cols = -c(펀드설명,기준일자),names_to = "symbol" ,values_to = "weight",values_drop_na = TRUE) %>%
      left_join(MP_VP_BM_results_descriptrion %>%
                  select(종목=ISIN,symbol)%>% distinct()) %>%
      asset_classification_and_adjust() %>% 
      left_join(all_tickers_daily_return , by = join_by(기준일자,종목==ISIN,자산군_대,자산군_소)) %>% 
      group_by(기준일자,펀드설명,자산군_대) %>% 
      reframe(전일대비등락률 = sum(weight*전일대비등락률) ) %>% 
      mutate(name = "VP"),
    ...
    MP_VP_BM_results_core %>%
      ...
      unnest_wider(`Weight_fixed(T)`) %>%
      pivot_longer(cols = -c(펀드설명,기준일자),names_to = "symbol" ,values_to = "weight",values_drop_na = TRUE) %>% 
      mutate(자산군_대 = if_else(str_detect(symbol,c("M2WD Index")),"주식","채권")) %>% 
      left_join(MP_VP_BM_results_raw %>% 
                  filter(port == "BM") %>%
                  filter(port == "BM") %>%
                  select(펀드설명,기준일자,daily_return_list) %>% 
                  unnest_wider(daily_return_list) %>% 
                  select(-펀드설명) %>% 
                  group_by(기준일자) %>% 
                  reframe(across(.cols = everything(),.fns = ~max(.x,na.rm = T))) %>% 
                  pivot_longer(cols = -기준일자,names_to = "symbol",values_to = "전일대비등락률"),
                by = join_by(기준일자,symbol)) %>% 
      group_by(기준일자,펀드설명,자산군_대) %>% 
      reframe(전일대비등락률 = sum(weight*전일대비등락률) ) %>% 
      mutate(name = "BM")
...
brinson_preprocess <- function(fund_desc_a, name_a, fund_desc_b, name_b) {
  ...
  weight_information_Port_AB <-bind_rows(
    ...
    mutate(across(.cols = -기준일자, .fns = ~lag(.x))) %>% 
    timetk::pad_by_time(.date_var = 기준일자,.by = "day",.fill_na_direction = "up")
...
# to_when <- "2024-07-18"
# brinson_processed_data <- brinson_preprocess("TIF", "MP", "TIF", "VP")
