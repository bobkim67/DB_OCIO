...
AP_VP_MP_diff_PA_weight <-
  ...
  left_join(
    left_join(MP_LTCMA_all_comb %>%
              ...
              BM_weight_all_comb %>%
                mutate(자산군_대 = if_else(ISIN %in% c("M2WD Index"),"주식","채권")) %>%
                group_by(기준일자, 펀드설명, 자산군_대) %>%
...
brinson_preprocess <- function(fund_desc_a, name_a, fund_desc_b, name_b) {
  # fund_desc_a="Golden Growth"
  # name_a="AP"
  # fund_desc_b ="MS GROWTH"
  # name_b = "BM"
...
brinson_results <- function(processed_data, from_when, to_when) {
  ...
  result_data <- processed_data %>% 
    ...
    select(-c(채권_Port_a_수익률, 채권_Port_b_수익률, Port_a_채권_weight, Port_b_채권_weight,Port_a_유동성_weight,Port_b_유동성_weight)) %>% 
    filter(기준일자 >= from_when & 기준일자 <= to_when) %>% 
    mutate(across(.cols = contains("_weight"), .fns = ~cummean(.x))) %>% 
    ...
    mutate(R_Port_a_Transaction_base = Actual_Port_a_수익률 - R_Port_a_Holding_base,
           R_Port_b_Transaction_base = Actual_Port_b_수익률 - R_Port_b_Holding_base,
...
brinson_summary<- function(brinson_result){
  ...
  bind_cols( tibble(구분 = c("포트폴리오","Transaction base 수익률","Holdiing base 수익률","주식 기여수익률","대체 기여수익률","채권 및 유동성 기여수익률")),
             PA_results %>% 
  ...
  ) %>% select(1,3,5) %>% setNames(c("구분","Port a","Port b")) %>% 
    mutate(across(.cols = -구분, .fns = ~scales::percent(.x,accuracy = 0.01)))-> summary1
